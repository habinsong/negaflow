...
extension AppModel {
    ...
    func recordDevelopHistory(for frame: ScanFrame) -> UUID {
        ...
        let entry = frame.makeDevelopHistoryEntry(label: text(AppLocalizedPhrase.historyNameFormat, nextNumber))
        frame.developHistory.append(entry)
        statusMessage = text(AppLocalizedPhrase.historyRecordedFormat, entry.label)
        return entry.id
    }
    ...
    func applyDevelopHistory(_ entry: DevelopHistoryEntry, to frame: ScanFrame) {
        ...
        statusMessage = text(AppLocalizedPhrase.historyAppliedFormat, entry.label)
        Task {
