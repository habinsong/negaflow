...
/// 포인트 커브 4채널: RGB(휘도·계조), Red, Green, Blue. 빈 배열 = 직선(identity).
...
/// HSL 8색 순서: 빨강·주황·노랑·초록·바다색(aqua)·파랑·자주(purple)·자홍(magenta).
...
public struct ColorMixer: Codable, Sendable, Equatable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        hue = Self.normalized(try c.decodeIfPresent([Double].self, forKey: .hue) ?? [])
        saturation = Self.normalized(try c.decodeIfPresent([Double].self, forKey: .saturation) ?? [])
        luminance = Self.normalized(try c.decodeIfPresent([Double].self, forKey: .luminance) ?? [])
    }
    ...
    private static func normalized(_ values: [Double]) -> [Double] {
        Array(values.prefix(MixerBand.allCases.count))
            + Array(repeating: 0, count: max(0, MixerBand.allCases.count - values.count))
    }
}
