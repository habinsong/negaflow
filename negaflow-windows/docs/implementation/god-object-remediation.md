...
| `src/Shell/Views/DevelopWorkspaceView.xaml.cs` | 3,294 | 미해결. 크롭·메타데이터·출력·버전·프리셋·필름 룩·소스 사이드바는 분리했다. 인스펙터·캔버스·GrainMend 가 남는다 |
| `tests/Native.UnitTests/develop_export_abi_tests.cpp` | 4,107 | 미해결 |
| `src/Shell/Views/LibraryWorkspaceView.xaml.cs` | 2,835 | 미해결 |
| `src/Shell/Views/DevelopWorkspaceView.xaml` | 1,591 | 미해결. 소스 사이드바 XAML 은 `Views/Develop/Sources/` 로 옮겼다. 인스펙터·캔버스·GrainMend 가 남는다 |
...
| P0 | `src/Shell/Views/DevelopWorkspaceView.xaml.cs` | 3,294 | 선택·미리보기·crop·GrainMend·metadata·inspector·resize·localization 이벤트/상태 | macOS surface 단위 실제 UserControl | 진행 중. 출력·버전·프리셋·필름·소스 사이드바를 옮겼다. 코드비하인드 4,844→3,294, XAML 2,512→1,591. 아직 500줄 초과 |
...
| P0 | `src/Shell/Views/DevelopWorkspaceView.xaml` | 1,591 | canvas/crop, histogram/tabs, 전체 inspector, GrainMend, metadata UI | macOS surface 단위 실제 UserControl; 공유 상태를 partial로 숨기지 않음 | 진행 중. 소스 사이드바 XAML 분리. 아직 500줄 초과 |
