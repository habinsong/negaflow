...
| 설정·다국어·접근성 | **95%** | 6개 로케일 전면 적용(문자열은 전부 macOS 표에서 생성), 설정 창 여덟 탭, 외관, 패널 너비·필름스트립 상태 저장. 작업 흐름 단축키가 macOS 이름·macOS 키로 돌고 설정의 단축키 표에서 바꿉니다. **macOS 설정 항목을 탭별로 대조해** 빈 스캔 탭(기본 스캔 회전·스캐너 성능)과 워크플로의 미세 반점 기본값을 채웠습니다. 남은 것: 픽셀 샘플러(현상 캔버스 기능이 먼저 필요), 언어 고르개(문자열은 6종 다 있으나 실행 중 전환은 미구현) |
...
**전체 어림 97%** 입니다. 인화를 포함한 값이며, 확인하지 않은 것(순수 ARM64 실행,
