...
| 설정·다국어·접근성 | **88%** | 6개 로케일 전면 적용(문자열은 전부 macOS 표에서 생성), 설정 창, 외관, 패널 너비·필름스트립 상태 저장. **작업 흐름 단축키**가 macOS 이름·macOS 키로 돌고, 설정의 단축키 표에서 묶음별로 바꾸고 되돌립니다. 남은 것: macOS 설정 항목 대조, 인화·전체 화면 등 아직 없는 화면의 단축키 |
...
**전체 어림 96%** 입니다. 인화를 포함한 값이며, 확인하지 않은 것(순수 ARM64 실행,
