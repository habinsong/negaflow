...
| 영역 | 대략 | 근거와 남은 것 |
| --- | --- | --- |
| 현상 엔진·수학 | **85%** | 반전·base(auto/manual/preset)·톤·포인트 커브·컬러 믹서·컬러 그레이딩·캘리브레이션·색·질감·FilmScanDenoise·필름 42종·GrainMend 수리·흑백 토닝·소프트 프루프·기하 변형까지 실제 스캔으로 확인. 남은 것: JPEG·DPI·리사이즈·출력 선명도, GrainMend **검출기**가 ABI 에 없음 |
| 카탈로그·영속성 | **90%** | 모든 recipe, 버전, 결함 sidecar, 프리셋, 붙여넣기 범위, 사용자 프리셋, 지연 저장, 백업·복구, relink, 단일 작성자 잠금. 남은 것: 앱/촬영/롤 메타데이터 |
| 라이브러리 화면 | **75%** | 카드 격자·정렬·카드 크기·소스 막대·파일 트리·폴더 프로세스 선택기·별점·깃발·가져오기·relink·중복 후보. 남은 것: 필터 3종(현재 롤·미검증 프로파일·메타데이터 미상), 컬렉션 |
| 현상 화면 | **70%** | 캔버스·히스토그램·인스펙터 6탭·보정 8섹션 전부·좌측 5탭·크롭/기하·자동 보정·프리셋/복사붙여넣기·정보 탭·GrainMend 브러시/복제 도장. 남은 것: 출력 품질·소스 하위 탭, 빠른 내보내기, GrainMend 자동·가이드, 메타데이터 카드 3종, 파일 탭 |
...
**GrainMend 탭에 도구가 생겼습니다 — 브러시와 복제 도장.** macOS 와 같은 알약 넷과 도구별
초기화 단추입니다. 자동·가이드는 검출기가 아직 네이티브 ABI 에 없어 이유를 달아 꺼 두었습니다
(`grain_mend_detector.h` 는 내부 헤더입니다). 브러시·복제 도장은 사용자 획을 그대로 싣는
경로라 네이티브 변경 없이 붙였습니다.
