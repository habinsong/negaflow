...
| 자동 보정 | 네이티브·ABI·셸 연결 완료 | ABI `nf_auto_adjust_v1`(0.29)은 미리보기 BGRA8 을 그대로 받는 분석 호출이라 새 요청 struct 가 없습니다. `AutoAdjustCoordinator` 가 톤·warmth/tint 를 0 으로 되돌린 사본을 렌더해 재고 결과를 **대입**합니다(누적 아님). Interop 147, Shell 325 assertions. WinUI 버튼은 UI 단계 |
