...
| 스캐너 | **78%** | 플러그인 클라이언트·발견·프로세스 호스트·프로토콜 v2·artifact 트랜잭션·게시 영수증에 더해 `ScanSessionController`·`ScannerPluginTrustStore`·`ScanStorageLayout` 이 붙어 라이브러리 가져오기 탭에서 검출→capability→스캔→카탈로그 게시가 이어집니다. 시뮬레이터로 검출→스캔→게시 전 구간을 하드웨어 없이 돌립니다. 평판 프레임 규격·프레임 자리·프레임별 스캔 영역도 붙었습니다. 남은 것: **실장치 검증 0회**, 프리뷰 픽셀 캡처(자동 프레임 찾기의 UI 연결) |
