...
| 라이브러리 화면 | **97%** | 카드 격자·정렬·카드 크기·소스 막대·파일 트리·폴더 프로세스 선택기·별점·깃발·가져오기·relink·중복 후보·다중 선택. **필터 9개 전부**를 macOS 차례로 냅니다. 수동 컬렉션을 만들고 고르고 지웁니다. 스마트 컬렉션·저장된 검색도 만들고 고르고 지웁니다. **카드 오른쪽 단추 메뉴**가 macOS 차례로 붙었습니다 — 스택·현상·이름 변경·별점·선택·제외·컬렉션 추가/제거·가상 사본·탐색기에서 보기·제거. 사진 이름도 macOS 규칙(사진 번호·`_frame_<n>`·롤 순번·사본 번호)을 따릅니다. **스택**(접기·펼치기·해제, 배지, 대표 투영), **가상 사본**, **원본을 폴더로 끌어 옮기기**(되돌리는 트랜잭션)까지 붙었습니다. 남은 것: 실행 취소 관리자 |
...
| 설정·다국어·접근성 | **84%** | 6개 로케일 전면 적용(문자열은 전부 macOS 표에서 생성), 설정 창, 외관, 패널 너비·필름스트립 상태 저장. **작업 흐름 단축키**가 macOS 이름·macOS 키로 돌고, 설정의 단축키 표에서 묶음별로 바꾸고 되돌립니다. 남은 것: macOS 설정 항목 대조, macOS 57개 중 아직 Windows 에 없는 명령들(비교/설문 보기 등)의 단축키 |
| 인화 | **5%** | 자리만 있습니다(312줄). 사용자가 뒤로 미룬 영역입니다 |
| ARM64·CI | **50%** | CI 에 native·managed·arm64 cross 잡이 있습니다. **순수 ARM64 실행은 하드웨어가 없어 확인 못 함** |

**전체 어림 79%**, 사용자가 우선하라고 한 **라이브러리+현상 범위로는 약 92%** 입니다.
