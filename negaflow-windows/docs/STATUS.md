...
| 내보내기 | **88%** | PNG16/8·TIFF16/8·JPEG8 을 폴더·파일명 패턴(토큰 7종)으로. 품질 탭에서 DPI·긴 변·TIFF 압축·비트 심도·JPEG 품질·출력 선명도·선명도 매체를 고르며 고른 값이 실제 파일에 나타납니다(실촬영 확인). 빠른 내보내기, 여러 장 배치, JSON·XMP 사이드카, 원본 보관, 이름 붙인 설정 프리셋. 세 포맷 모두 WIC sRGB ICC·DPI·구조/픽셀 readback 과 비덮어쓰기 게시를 거칩니다. 설정 → 내보내기 탭의 색상 관리 구획에서 출력 색공간을 고르며 실제 파일의 픽셀과 ICC 가 따라옵니다(실촬영 확인). 남은 것: alpha, 메타데이터 정책, MAIN 무보정본, 프리셋 이름 변경 |
| 스캐너 | **85%** | 플러그인 클라이언트·발견·프로세스 호스트·프로토콜 v2·artifact 트랜잭션·게시 영수증에 더해 `ScanSessionController`·`ScannerPluginTrustStore`·`ScanStorageLayout` 이 붙어 라이브러리 가져오기 탭에서 검출→capability→스캔→카탈로그 게시가 이어집니다. 시뮬레이터로 검출→스캔→게시 전 구간을 하드웨어 없이 돌립니다. 평판 프레임 규격·프레임 자리·프레임별 스캔 영역도 붙었습니다. 프리뷰 픽셀 캡처와 자동 프레임 찾기도 UI 에서 돕니다. 합성 스트립에서 검출기가 다섯 컷을 정확한 치수로 찾는 것까지 확인했습니다. 남은 것: **실장치 검증 0회**, 실제 필름 프리뷰(먼지·퍼포레이션·기울어짐)에서의 검출 |
