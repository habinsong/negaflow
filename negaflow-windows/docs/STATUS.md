...
| 내보내기 | **70%** | PNG16·TIFF16·JPEG8 을 폴더·파일명 패턴(순번 포함)으로. 출력 패널의 파일·품질·소스 하위 탭에서 DPI·긴 변·TIFF 압축·JPEG 품질·출력 선명도·선명도 매체를 고르며, 고른 값이 실제 요청에 실립니다. 빠른 내보내기 절과 여러 장 배치, 그리고 JSON·XMP 사이드카와 원본 보관도 동작합니다. 세 포맷은 WIC sRGB ICC·DPI·구조/픽셀 readback 과 비덮어쓰기 게시를 거칩니다. 남은 것: 출력 색공간, PNG/TIFF 8-bit·alpha, 내보내기 설정 프리셋, 메타데이터 정책, MAIN 무보정본, 배치 사이드카 |
