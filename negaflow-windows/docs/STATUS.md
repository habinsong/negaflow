...
| 영역 | 대략 | 근거와 남은 것 |
| --- | --- | --- |
| 현상 엔진·수학 | **92%** | 반전·base(auto/manual/preset)·톤·포인트 커브·컬러 믹서·컬러 그레이딩·캘리브레이션·색·질감·FilmScanDenoise·필름 42종·GrainMend 수리·검출(자동·가이드 ROI·미세 입자)·흑백 토닝·소프트 프루프·기하 변형·긴 변 Lanczos3·출력 선명도·PNG/TIFF/JPEG 게시·TIFF LZW/Deflate·PNG/TIFF 8-bit 과 macOS 출력 디더·출력 색공간(sRGB/Display P3/Adobe RGB, ICC 생성). 남은 것: alpha, 색역 클리핑, **macOS 픽셀 대조 — R 채널 +975**. 장면 통계 표본을 면적 평균으로 고치고 표본 격자를 macOS 와 맞춰 G·B 차이가 열 배 줄었고(G max 는 60416 으로 정확히 일치), R 만 반대쪽에 남았습니다. base 와 어두운 비필름 게이트는 둘 다 원인이 아님을 측정으로 확인했습니다. **8100 흑화는 고쳤습니다** — 연결 성분 전략이 증거가 약할 때 포기하도록 관문을 넣어 median 19 → 23782(macOS 25353), base 0.0320 → 0.1764(macOS 0.1913). V700 은 바이트 동일입니다. `docs/verification/2026-08-16-macos-pixel-golden.md` |
...
| 내보내기 | **93%** | PNG16/8·TIFF16/8·JPEG8 을 폴더·파일명 패턴(**macOS 와 같은 토큰 9종**)으로. `{name}` 은 macOS 처럼 카드에 보이는 이름입니다 — 원본 파일 이름이 아닙니다. 품질 탭에서 DPI·긴 변·TIFF 압축·비트 심도·JPEG 품질·출력 선명도·선명도 매체를 고르며 고른 값이 실제 파일에 나타납니다(실촬영 확인). 빠른 내보내기, 여러 장 배치, JSON·XMP 사이드카, 원본 보관, 이름 붙인 설정 프리셋. 세 포맷 모두 WIC sRGB ICC·DPI·구조/픽셀 readback 과 비덮어쓰기 게시를 거칩니다. 설정 → 내보내기 탭의 색상 관리 구획에서 출력 색공간을 고르며 실제 파일의 픽셀과 ICC 가 따라옵니다(실촬영 확인). 메타데이터 정책 네 가지가 TIFF·JPEG 실파일에 닿습니다. 남은 것: alpha, 원본 메타데이터 복사, MAIN 무보정본, 프리셋 이름 변경 |
