...
| 현상 화면 | **90%** | 캔버스·히스토그램·인스펙터 6탭·보정 8섹션 전부·좌측 6탭(파일 탭 포함)·크롭/기하·자동 보정·프리셋/복사붙여넣기·GrainMend 네 도구·정보 탭의 메타데이터 카드 3종(앱·촬영·롤)·출력 패널의 파일/품질/소스 하위 탭과 빠른 내보내기. Base 구획이 macOS 와 같은 세 줄입니다 — 필름·광원·**스캐너 프로파일**. 남은 것: macOS 설정 항목 세부 대조, 소스 탭의 메타데이터 정책·MAIN 무보정본 |
...
| 설정·다국어·접근성 | **84%** | 6개 로케일 전면 적용(문자열은 전부 macOS 표에서 생성), 설정 창, 외관, 패널 너비·필름스트립 상태 저장. **작업 흐름 단축키**가 macOS 이름·macOS 키로 돌고, 설정의 단축키 표에서 묶음별로 바꾸고 되돌립니다. 남은 것: macOS 설정 항목 대조, macOS 57개 중 아직 Windows 에 없는 명령들(가상 사본·비교/설문 보기·현상 대상 전환)의 단축키 |
| 인화 | **5%** | 자리만 있습니다(312줄). 사용자가 뒤로 미룬 영역입니다 |
| ARM64·CI | **50%** | CI 에 native·managed·arm64 cross 잡이 있습니다. **순수 ARM64 실행은 하드웨어가 없어 확인 못 함** |

**전체 어림 78%**, 사용자가 우선하라고 한 **라이브러리+현상 범위로는 약 91%** 입니다.
