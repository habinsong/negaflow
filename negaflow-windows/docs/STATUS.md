...
- **GrainMend 수리가 아무 화소도 고치지 않는 근본 원인을 찾았다 — 마스크 화소 폭 불일치.** `repair_defect_components`는 마스크를 화소당 1바이트로 읽는다(`mask[y * mask_stride_bytes + x]`). 그런데 region 편집이 싣는 마스크는 RGBA8, 즉 화소당 4바이트다. 실측 `mask_bytes=69,217,152 (5088×3401×4)`, `stride=20352 (5088×4)`. 그래서 커널은 각 행 20,352바이트 중 앞 5,088바이트만 읽고, 화소 0~1,271의 R·G·B·A 를 화소 0~5,087의 가중치로 오해한다. 행의 3/4는 읽히지도 않는다. 그 앞부분에 먼지가 없으면 `weight > 8`이 하나도 없어 `region_repaired_px=0`이 되고, 이것이 관측값과 정확히 일치한다. `valid_mask_layout`은 `stride >= width`만 확인하므로 이 어긋남을 통과시켜 여태 오류 없이 `ok`만 반환했다. 아직 고치지 않았다 — 1바이트 마스크를 보내는 다른 경로(IR·시험)를 함께 확인해야 어느 쪽을 맞출지 정할 수 있다.
- **preview 와 export 가 같은 recipe 를 쓰지 않는다 — 원본 해상도에서 갈라짐을 확정했다.** 같은 두 요청(recipe 있음/없음)으로 잰 값이다: **export 5088×3401 은 3,317,051 바이트가 다르고**(수리가 실제로 일어난다), **preview 는 900×700·1600×1200·2400×1800·5088×3401 네 크기 모두 0 바이트**다. 원본 크기 preview 조차 같으므로 축소에 씻겨 나간 것이 아니라 preview 경로가 결함 결과를 버린다. 관리 측 마샬링은 같고(`BuildDefectRegionPayload` 공유), ABI 의 `map_request_v34` 두 번째 인자는 `require_destination` 이라 결함과 무관하며, `develop_and_export` 와 `develop_preview` 는 같은 `run_develop` 을 부른다. 아직 정확한 줄은 못 짚었다 — 다음 탐침은 `run_develop` 안에서 defect 단계 뒤 `decoded_image` 가 preview 분기에서 다시 덮어써지는지다.
...
> `policy-all.tif`를 source로 실제 Windows TIFF 네 정책의 root IFD 문자열 값·보존/제거를 회귀화했고,
> Task 7은 CIUnsharpMask와 여섯 CIGaussianBlur 반경을 회귀 상한 0.008로 연결했고, Task 8도
> 0.005 상한으로 연결했습니다. Task 9 B&W는 자동 base의
> 크로모제닉 재시도 누락을 고쳐 7개 중앙값을 64 RGB16 코드 이내로 회귀화했습니다. Task 1은
> 저장소 밖에서 제공된 GT-X900 원본의 SHA-256을 manifest와 대조한 뒤 실제 ABI 재렌더를 수행해
> 여덟 조건의 RGB16 중앙값을 96 코드 이내로 회귀화했습니다. B&W Noritsu의 국소 최대 차이
> 1,093 코드와 Task 1 Portra 400의 국소 최대 차이 6,482 코드는 남아 있습니다. 상세 명령·수치는
> `verification/2026-08-16-request-5-windows-golden.md`를 우선합니다.

| 영역 | 대략 | 근거와 남은 것 |
| --- | --- | --- |
| 현상 엔진·수학 | **98%** | 반전·base(auto/manual/preset)·톤·포인트 커브·컬러 믹서·컬러 그레이딩·캘리브레이션·색·질감·FilmScanDenoise·필름 42종·GrainMend 수리·검출·흑백 토닝·소프트 프루프·기하 변형·긴 변 Lanczos3·출력 선명도·PNG/TIFF/JPEG 게시·TIFF LZW/Deflate·8-bit 디더·출력 색공간(sRGB/Display P3/Adobe RGB, ICC 생성). `CIVibrance`는 Apple 내장이라 수식을 되짚지 않고 33³ 일반 response 표(211 KiB)와 17개 slider-range plane으로 회귀화했습니다. Task 7은 CIUnsharpMask와 CIGaussianBlur 여섯 반경을 같은 RGBAf golden으로 확인했고, 1.0/1.3 실제 내부 사용처도 같은 사상을 공유합니다. V700 실제 ABI 재렌더 여덟 조건은 RGB16 중앙값이 모두 96 코드 이내이며, `CIToneCurve`의 감마 2 지각 spline을 ScannerProfileGrade에 반영해 Portra 400 중앙값도 `−18/+9/+6`까지 수렴했습니다. spline 계수는 프로파일 적용마다 한 번만 계산하며, 여덟 조건 재렌더 결과가 같은지 확인했습니다. 남은 것: Task 1 Portra 400의 국소 최대 6,482 RGB16 코드와 Task 9 B&W Noritsu의 1,093 코드이며, 둘 다 pixel-exact 동등성 주장은 하지 않습니다. `docs/verification/2026-08-16-request-5-windows-golden.md` |
...
| 라이브러리 화면 | **99%** | 카드 격자·정렬·카드 크기·소스 막대·파일 트리·폴더 프로세스 선택기·별점·깃발·가져오기·relink·중복 후보·다중 선택. **필터 9개 전부**를 macOS 차례로 냅니다. 수동 컬렉션을 만들고 고르고 지웁니다. 스마트 컬렉션·저장된 검색도 만들고 고르고 지웁니다. **카드 오른쪽 단추 메뉴**가 macOS 차례로 붙었습니다 — 스택·현상·이름 변경·별점·선택·제외·컬렉션 추가/제거·가상 사본·탐색기에서 보기·제거. 사진 이름도 macOS 규칙(사진 번호·`_frame_<n>`·롤 순번·사본 번호)을 따릅니다. **스택**(접기·펼치기·해제, 배지, 대표 투영), **가상 사본**, **원본을 폴더로 끌어 옮기기**(되돌리는 트랜잭션)까지 붙었습니다. **되돌리기·다시 실행**(Ctrl+Z / Ctrl+Shift+Z)과 macOS 의 세 훑어보기 모드(격자·비교·살펴보기)까지 붙었습니다. 제거 확인 대화상자는 되돌리기가 대신하므로 없앴습니다 — macOS 와 같습니다 |
| 현상 화면 | **96%** | 캔버스·히스토그램·인스펙터 6탭·보정 8섹션 전부·좌측 6탭(파일 탭 포함)·크롭/기하·자동 보정·프리셋/복사붙여넣기·GrainMend 네 도구·정보 탭의 메타데이터 카드 3종(앱·촬영·롤)·출력 패널의 파일/품질/소스 하위 탭과 빠른 내보내기. Base 구획이 macOS 와 같은 세 줄입니다 — 필름·광원·**스캐너 프로파일**. 라이브러리 사이드바에 macOS 와 같은 **현상 구획**(프로세스·타깃 다섯 칸·필름 프로파일·룩)이 붙어 **현상 타깃 일곱 가지**를 고를 수 있습니다. 출력 패널 소스 탭에 **메타데이터 정책 네 가지**·MAIN 무보정본·원본 보관·사이드카가 모두 있습니다. 설정 항목의 탭별 대조는 완료했습니다. 남은 것: 없음 |
| 내보내기 | **93%** | PNG16/8·TIFF16/8·JPEG8 을 폴더·파일명 패턴(**macOS 와 같은 토큰 9종**)으로. `{name}` 은 macOS 처럼 카드에 보이는 이름입니다 — 원본 파일 이름이 아닙니다. 품질 탭에서 DPI·긴 변·TIFF 압축·비트 심도·JPEG 품질·출력 선명도·선명도 매체를 고르며 고른 값이 실제 파일에 나타납니다(실촬영 확인). 빠른 내보내기, 여러 장 배치, JSON·XMP 사이드카, 원본 보관, 이름 붙인 설정 프리셋. 세 포맷 모두 WIC sRGB ICC·DPI·구조/픽셀 readback 과 비덮어쓰기 게시를 거칩니다. 설정 → 내보내기 탭의 색상 관리 구획에서 출력 색공간을 고르며 실제 파일의 픽셀과 ICC 가 따라옵니다(실촬영 확인). 메타데이터 정책 네 가지가 TIFF·JPEG 실파일에 닿으며, **원본 메타데이터를 정책대로 걸러 옮깁니다** — macOS `ExportSourceMetadata` 와 같은 규칙으로 TIFF·EXIF·IPTC·GPS 를 옮기고, 픽셀과 어긋날 수 있는 구조 태그는 옮기지 않습니다. 네 정책은 macOS `policy-all.tif` 실제 source로 root 문자열 값과 EXIF·IPTC·GPS 보존/제거를 회귀화했습니다. `preserveAlpha`는 ABI v34부터 Shell·sidecar·PNG/TIFF 8·16bit 구조까지 닫았습니다. 남은 것: Task 6의 태그 순서와 IPTC IIM/XMP 바이트 단위 동일성은 검증 범위가 아닙니다. |
| 스캐너 | **98%** | 플러그인 클라이언트·발견·프로세스 호스트·프로토콜 v2·artifact 트랜잭션·게시 영수증에 더해 `ScanSessionController`·`ScannerPluginTrustStore`·`ScanStorageLayout` 이 붙어 라이브러리 가져오기 탭에서 검출→capability→스캔→카탈로그 게시가 이어집니다. 시뮬레이터로 검출→스캔→게시 전 구간을 하드웨어 없이 돌립니다. 평판 프레임 규격·프레임 자리·프레임별 스캔 영역도 붙었습니다. 프리뷰 픽셀 캡처와 자동 프레임 찾기도 UI 에서 돕니다. 합성 스트립에서 검출기가 다섯 컷을 정확한 치수로 찾는 것까지 확인했습니다. **실기 검증 완료 — OpticFilm 8100 과 Epson V700 두 대가 detect→capabilities→scan 전 구간을 돕니다.** 플러그인이 SANE 기기표에서 생성한 332종 INF 로 자립하며 벤더 소프트웨어에 기대지 않습니다. 실제 필름 프리뷰 10장에서 적재량과 검출 컷 수가 일치합니다. **적외선이 검출부터 복원까지 실기로 닫혔습니다** — V700 이 IR 채널을 내고, 검출이 먼지를 찾고(정렬 오차 1화소), 복원의 큰 변화가 결함 면적과 세 자리까지 일치합니다. 검출 좌표를 그대로 스캐너에 넣어 필름이 나오는 것까지 확인했습니다. **macOS 골든과 정렬·검출이 전부 일치합니다**(coverage 0.1006%, offset (1,−1), peak 0.012, candidates/confirmed 120/59). 남은 것: 없음 |
| 설정·다국어·접근성 | **99%** | 6개 로케일 전면 적용(문자열은 전부 macOS 표에서 생성), 설정 창 여덟 탭, 외관, 패널 너비·필름스트립 상태 저장. 작업 흐름 단축키가 macOS 이름·macOS 키로 돌고 설정의 단축키 표에서 바꿉니다. **macOS 설정 항목을 탭별로 대조해** 빈 스캔 탭(기본 스캔 회전·스캐너 성능)과 워크플로의 미세 반점 기본값을 채웠습니다. **픽셀 샘플러**와 **언어 고르개**까지 붙어 macOS 설정 항목이 모두 있습니다. 남은 것: 없음 |
| 인화 | **99%** | 용지 27종·방향·여백·해상도·천공(ISO 1007 135)·공정 겉모습(시아노타입·건판·젤라틴실버)·컨택트 시트·픽처 패키지 템플릿 3종·**커스텀 자유 배치**·**캡션 글자**·재단선·**눈금자**까지 macOS 와 **같은 수**로 계산하고 그립니다. 검사기와 캔버스 미리보기가 붙었고, 판을 PNG 로 씁니다(해상도 태그 포함). 실촬영 확인 완료. **커스텀 배치 편집기**(끌어 옮기기·모서리로 크기 조절)까지 붙어 macOS 인화 기능이 모두 있습니다. 남은 것: 없음 |
| ARM64·CI | **50%** | CI 에 native·managed·arm64 cross 잡이 있습니다. **순수 ARM64 실행은 하드웨어가 없어 확인 못 함** |

**남은 일 전부는 `docs/progress/remaining-2026-08-16.md` 한 장에 모아 두었습니다.** 맥이 있어야
하는 것은 `docs/verification/macos-golden/REQUEST-5.md` 입니다.

**전체 어림 98%** 입니다. 인화를 포함한 값이며, 확인하지 않은 것(순수 ARM64 실행)은 완료로
세지 않았습니다.

macOS 픽셀 대조는 더 이상 "맥이 없어서 못 한다" 가 아닙니다. 맥 쪽 에이전트가 골든을 세 차례
보내 주어 **8100 은 여섯 지점이 정확히 일치하고**, V700 도 G·B 는 1.4 레벨 안에 들어왔습니다.
남은 것은 R 한 채널이고, 그 자리를 `CIVibrance` 로 좁혔습니다. 그 필터는 Apple 내장이라 커널
소스가 없으므로 **수식을 되짚지 않고 잰 사상을 표로 넣습니다** — 33³ 격자를 기다리는 중입니다
(`docs/verification/macos-golden/REQUEST-4.md`).

## 2026-08-16 오늘 한 것

- **원본 메타데이터 복사** — 정책 네 가지가 값만 받고 원본을 열지도 않던 자리를 채웠습니다.
  TIFF·JPEG 실파일로 네 정책을 모두 확인했습니다. `docs/verification/2026-08-16-export-source-metadata.md`
- **EXIF 오프셋 태그 세 개**(`36880`·`36881`·`36882`, `+00:00`) — 없으면 읽는 쪽이 제 시간대로
  해석해 몇 시간씩 어긋납니다. macOS 와 맞췄습니다.
- **밉맵 축소기 통일** — 엔진이 읽는 모든 통계(장면 격자·base 격자·채도 프록시)가 한 축소기를
  씁니다. 8100 이 사진으로 나오게 된 변경입니다.
- **macOS 진단 이식** — `NEGA_DEBUG` 로 `[nega-proxy]`·`[nega]`·`[vib]` 세 줄을 macOS 와 같은
  이름·같은 자릿수로 냅니다. 이제 대조가 산술이 아니라 실측입니다.
- **재고 넣지 않은 것 하나** — vibrance 앵커. 틀린 것은 확실하지만 그것만 고치면 RMS 가
  707 → 703 이라 픽셀만 바뀌고 나아지지 않습니다. 표가 올 때 배율과 함께 바꿉니다.

## 2026-08-15 픽셀 샘플러와 언어 고르개

설정에서 macOS 에 있고 여기 없던 마지막 둘입니다.

**픽셀 샘플러**가 포인터 아래 화소의 값을 읽습니다. macOS 는 원본·작업·프루프 셋을 나란히
내는데, 여기서는 **원본**과 **화면에 보이는 것** 둘입니다 — Windows 미리보기는 한 번에 버퍼
하나만 만들고 프루프를 켜면 그 버퍼가 곧 프루프 결과입니다. 셋을 다 내려면 슬라이더를 움직일
때마다 렌더가 두 배가 되므로, **보이는 줄이 지금 무엇인지를 이름으로 밝히는** 쪽을 골랐습니다.
둘 다 보이는 척하지 않습니다.

원본 화소는 프레임마다 한 번 읽어 둡니다. 읽는 사이에 다른 사진으로 옮겼으면 늦게 온 결과를
버립니다 — 그렇지 않으면 다른 사진의 값이 이 사진의 값으로 보입니다.

RGB 옆에 **Lab** 을 적습니다. 색을 눈이 아니라 수로 견주는 것이 샘플러를 두는 이유입니다.
시험이 흰색 L=100, 검정 L=0, 중성 무채도, 빨강의 큰 양수 a 를 붙들고 있습니다 — 부호가 뒤집히면
색을 정반대로 보고하게 됩니다.

**언어 전환은 다시 시작할 때 걸립니다.** WinUI 는 리소스를 실행당 한 번 고르므로, 켜져 있는
창 밑에서 바꾸면 이미 만들어진 컨트롤이 옛 언어를 들고 있어 한 화면에 두 언어가 섞입니다.
고르개가 그 사실을 말합니다 — 실패한 것처럼 보이게 두지 않습니다. 목록에는 **리소스가 있는
여섯**만 올립니다. 없는 언어를 올리면 고른 순간 화면이 영어로 돌아갑니다.

## 2026-08-15 설정 항목을 탭별로 맞췄습니다

macOS 설정 창과 탭 하나씩 대조했습니다. 세 자리가 비어 있었습니다.

**스캔 탭이 통째로 비어 있었습니다.** 제목만 있고 아무 항목도 없었습니다. 채운 둘:

- **기본 스캔 회전.** 홀더에 필름을 늘 같은 방향으로 넣는 사용자가 매번 돌리지 않도록 하는
  값입니다. 게시할 때 **recipe 에만** 적히고 원본 파일은 건드리지 않습니다. 프리뷰 스캔에는
  걸지 않습니다 — 프리뷰는 프레임을 찾으려고 뜨는 것이라, 돌리면 찾은 좌표가 실제 판의 좌표와
  어긋납니다.
- **스캐너 성능.** 장치가 보고한 것만 적습니다. 여기 앱이 계산한 값을 두면 사용자는 그것을
  장치의 사양으로 읽습니다.

**워크플로에 미세 반점 기본값이 없었습니다.** 새로 여는 결함 도구가 가장 작은 반점까지 찾기로
시작할지를 정합니다. 켬으로 박혀 있었고, 프레임마다 고른 값은 여전히 우선합니다.

**이 작업에서 남길 것.** 시험 두 개가 코드보다 먼저 틀렸습니다. 회전 시험이 처음에 "params 가
비어 있어야 한다" 고 요구했는데 develop route 가 그 뒤에 자기 값을 적습니다 — 없어야 하는 것은
`imageTransform` 이었습니다. 회전 자체도 `params` 바로 밑이 아니라 `imageTransform` 안에 사는데,
잘못 적었다면 reader 가 조용히 무시했을 것입니다. **적히는 자리를 reader 에서 확인하고 쓸 것.**

## 2026-08-15 커스텀 배치 편집기와 메타데이터 정책

**커스텀 배치를 손으로 놓습니다.** 모델과 계산과 시험은 있었는데 배치할 방법이 없었습니다. 칸을
끌어 옮기고 모서리로 크기를 바꿉니다. 끄는 동안에도 **자리는 끝까지 비율**입니다 — 화소로
옮겼다가 마지막에 나누면 옮길 때마다 반올림이 쌓여, 반듯하던 배치가 한 세션 뒤에 어긋납니다.
칸은 내용 영역 안에 갇힙니다: 판 밖으로 나간 칸은 배치 전체를 거절당하므로, 끌다가 막는 편이
끌고 나서 거절당하는 것보다 낫습니다.

**메타데이터 정책 고르개가 없었습니다.** `export_metadata.cpp` 는 처음부터 네 가지를 냈고 고를
방법만 없었습니다 — 이 이식에서 반복해서 나온 그 모양입니다. macOS 와 같이 출력 패널의 소스
탭, 원본 보관·사이드카 옆에 둡니다.

실촬영으로 TIFF 넷을 써서 확인했습니다.

| 정책 | 저작권 | 소프트웨어 | 바이트 |
| --- | --- | --- | ---: |
| 최소 | 있음 | 있음 | 30,853,546 |
| 저작권만 | 있음 | **없음** | **30,853,524** |
| 위치 제거 | 있음 | 있음 | 30,853,546 |
| 전체 | 있음 | 있음 | 30,853,546 |

눈에 띄게 다른 것은 "저작권만" 입니다 — 소프트웨어 이름까지 빼서 22바이트가 짧습니다. 나머지
셋은 **원본** 태그 중 무엇을 옮기느냐만 다른데 이 스캔들에는 옮길 태그가 없으므로, 같은 결과가
나오는 것이 정상이며 설정이 안 걸린 것이 아닙니다.

**이 확인에서 남길 것.** 처음 재었을 때 네 정책 모두 "아무것도 안 씀" 으로 나왔습니다. TIFF 는
큰 이미지의 IFD 를 파일 **끝**에 두는데 앞 4KB 만 읽었기 때문입니다. 코드가 아니라 측정이
틀렸습니다 — GrainMend 때와 같은 모양이고, 이번에는 바이트 수는 다른데 문자열 검색은 같다고
말하는 모순이 그것을 잡아 주었습니다. **두 지표가 어긋나면 지표를 먼저 의심할 것.**

## 2026-08-15 캡션 글자·눈금자·커스텀 배치

인화의 남은 셋을 채웠습니다.

**캡션이 글자를 담습니다.** 자리만 잡아 두었던 것을 실제로 찍습니다. 글자는
`RenderTargetBitmap` 으로 굽는데, 이것이 XAML 이 화면에 글자를 그릴 때 쓰는 것과 같은 길입니다 —
그리기 엔진을 하나 더 들이면 미리보기의 캡션과 인화된 캡션이 다른 글꼴로 나올 수 있습니다.
글자는 제 알파로 섞습니다. 알파를 무시하면 글자마다 불투명한 네모가 남습니다.

캡션을 못 구우면 **캡션만 빠지고 사진은 나갑니다.** 글자 한 줄 때문에 인화 전체를 실패시키지
않습니다. 별점 0 은 빈칸이 아니라 줄표입니다 — 빈칸이면 캡션을 켰는지 껐는지 알 수 없습니다.

**눈금자는 용지의 실제 mm 를 따릅니다.** 화면 배율이 아닙니다. 그래야 화면에서 잰 길이가
인화물에서도 같습니다. 센티미터는 1cm 마다 숫자와 5mm 짧은 눈금, 인치는 1인치마다 숫자에
1/2·1/4 눈금입니다 — 1/8 까지 넣으면 미리보기에서 눈금이 서로 붙습니다.

**커스텀 배치는 자리를 비율로 담습니다.** 화소로 담으면 용지나 해상도를 바꾼 순간 배치가
흩어집니다. 시험이 A4 를 Letter 로 바꿔 그 비율이 살아남는지 붙들고 있습니다. 칸은 z 차례로
쌓고 같으면 목록 차례입니다.

**거절하는 세 가지.** 판 번호에 구멍이 있으면(1번만 있고 0번이 없으면) 첫 장이 빈 채로
인쇄되므로 배치를 아예 만들지 않습니다. 판 밖으로 나가는 칸과 없는 사진을 가리키는 칸도
같습니다 — 반쯤 그려 놓고 나중에 발견되는 편이 더 나쁩니다.

## 2026-08-15 고른 ICC 가 엔진까지 닿지 않고 있었습니다

소프트 프루프가 `ToSettings(null)` 이었습니다. 설정 화면은 ICC 를 고르게 하고 검사까지 했는데
**파일 이름만** 담아 두어, 측정된 용지 흰색과 잉크 검정은 한 번도 읽히지 않았습니다. "용지와
잉크" 를 골라도 조용히 "프로파일만" 으로 돌았습니다 — 컨트롤은 있는데 아무 일도 하지 않는,
이 이식에서 계속 나오는 그 모양입니다.

이제 자리를 담고, 미리보기가 필요할 때 읽습니다. 경로와 쓴 시각으로 캐시하므로 슬라이더를
움직일 때마다 파일을 다시 읽지 않습니다.

**인화 출력 프로파일은 따로 둡니다**(macOS 와 같음). 화면에서 보는 목적지와 종이에 찍는
목적지는 같지 않습니다. PRINT 로 현상하면 프루프 목적지가 프린터 프로파일로 바뀝니다.

이 기계의 실제 프로파일로 확인했습니다.

| 프로파일 | 결과 |
| --- | --- |
| sRGB Color Space Profile.icm | 받아들임 |
| AdobeRGB1998.icc | 받아들임 |
| RSWOP.icm | **거절** — CMYK 인쇄 프로파일은 RGB 프루프 목적지가 아닙니다 |

프로파일이 있으면 `simulation=PaperAndBlackInk`, 없으면 `ProfileOnly` 로 물러납니다. 목적지도
MAIN 은 프루프 프로파일, PRINT 는 프린터 프로파일로 갈립니다.

두 디스플레이 프로파일이 중립 흰색을 내는 것은 정상입니다 — 디스플레이 프로파일에는 측정된
용지가 없습니다. 여기서 확인된 것은 **모드와 목적지가 엔진까지 닿는다**는 것이지 수가
움직였다는 것이 아닙니다. 흰색을 물들이는 것은 실제 인화지 프로파일입니다.

## 2026-08-15 픽처 패키지·캡션·재단선

픽처 패키지 템플릿 셋이 macOS 비율 그대로입니다 — 큰 칸이 가로의 2/3 를 가지고 작은 두 칸이
옆에 쌓이거나, 좌우 2분할, 또는 2×2. 사진이 칸보다 적으면 앞에서부터 다시 씁니다. 빈 칸을
남기면 인화지 한 장이 반만 쓰입니다.

**캡션은 칸에서 자리를 가져가고 사진이 그만큼 물러납니다.** 그래서 캡션 자리가 판 계산의
일부이지 위에 덧그리는 것이 아닙니다. 칸 절반보다 크면 잘라 냅니다 — 사진이 제 이름표에
파묻히면 안 됩니다.

**재단선은 판을 넘지 않습니다.** 넘은 선은 잘려 나가 인쇄물에 반쪽만 남습니다. 사진 위에
얹고, 어두운 종이에서는 밝게 그립니다.

실촬영 4장, A4 300dpi 로 재었습니다.
...
| 2분할 | 판 2장, 칸 2개 |
| 4분할 | 판 1장, 칸 4개 |
| 컨택트 2×2 + 캡션 + 재단선 | 칸 4개, 재단선 16개, 캡션 71px |

71px 은 300dpi 의 6mm 입니다. 재단선 16개는 판 안에 드는 선분들이며, 나머지는 판을 넘어
**중간에 자르지 않고 통째로 뺐습니다**.

## 2026-08-15 인화

인화를 만들었습니다. 사용자가 뒤로 미뤄 두었던 영역이라 마지막에 왔습니다.

**기하가 먼저입니다.** 용지 27종의 mm 치수, 방향 결정, 여백, 해상도, 그리고 천공 배치까지
macOS 와 **같은 수**로 옮기고 시험으로 붙들었습니다. 천공은 ISO 1007 의 135 풀프레임 규격입니다 —
35mm 폭, 24×36mm 게이트, 4.75mm 피치로 한 쪽 8개씩 16개. 이 수가 어긋나면 같은 설정에서 다른
크기의 인화물이 나오고, 그것은 사용자가 눈으로 알아채기 가장 어려운 종류의 어긋남입니다.

**화면과 파일이 같은 `PrintCompositionLayout` 을 씁니다.** 미리보기를 따로 계산했다면 보이는
여백과 나오는 여백이 갈렸을 것이고, 그 차이는 인화를 마친 뒤에야 드러납니다.

**사진은 현상 엔진이 냅니다.** 인화가 색을 따로 만들면 현상 화면에서 본 것과 다른 사진이
인화됩니다. `PrintSheetWriter` 가 하는 일은 현상 결과를 layout 이 정한 자리에 놓는 것뿐이며,
크기 맞추기도 WIC 에 맡깁니다 — 직접 재표본화하면 내보내기의 긴 변 축소와 다른 결과가 납니다.

실제 V700 컬러 네거티브(1768×2906)로 확인했습니다.

| 설정 | 결과 |
| --- | --- |
| A4 300dpi | 캔버스 2480×3508, 이미지 1991×3272 |
| A4 + 35mm 천공 | 천공 16개, 이미지가 게이트 안으로 1404×2308 |
| 컨택트 4×3, 12장 | 판 1장·칸 12개, 캔버스 3508×2480(격자를 따라 가로) |
| 쓴 파일 | 2480×3508 @ **300dpi** |

세로 사진에 세로 용지가, 4×3 격자에 가로 용지가 자동으로 붙었습니다. 해상도를 파일에 적는
것이 중요합니다 — 인화소는 그 값으로 실제 크기를 정하므로, 빠뜨리면 300dpi 로 짠 판이 72dpi 로
인쇄됩니다.

**남은 것과 그 이유.** 픽처 패키지 템플릿과 커스텀 패키지의 자유 배치, 캡션과 크롭마크,
C-print 출력 프로파일, 눈금자가 아직입니다. 그중 **C-print 출력 프로파일**은 앞서 적은 PRINT
타깃이 MAIN 과 픽셀이 같았던 이유이며, 그것이 붙어야 PRINT 가 제 몫을 합니다.

## 2026-08-15 되돌리기, 그리고 비교·살펴보기

**되돌리기가 없어서 제거는 물어보는 조작이었습니다.** 대화상자는 되돌리기의 대용품이었고,
macOS 에는 그 물음이 없습니다 — 바로 빼고 되돌릴 수 있다고 알립니다. 이제 여기도 같습니다.

되돌리기는 연산마다 역연산을 쓰지 않고 **바뀌기 직전 상태를 통째로** 담습니다. 열 가지 연산은
열 가지 실수 자리를 만들지만 상태 복원은 한 자리뿐입니다. 카탈로그 한 장이 수 MB 이므로 20단계로
제한했습니다.

세 가지가 결정적이었습니다.

- **깊은 복사.** 얕게 담으면 이어지는 편집이 담아 둔 상태까지 바꿔 되돌리기가 아무 일도 하지
  않습니다. 노출값이 돌아오는지를 시험이 붙들고 있습니다.
- **자리까지 복원.** 끝에 다시 붙이면 "입력 순서" 정렬에서 사진이 옮겨 다닌 것으로 보입니다.
  묶음·스택 소속도 함께 돌아옵니다.
- **결함 sidecar 를 지우지 않습니다.** 되살아난 사진이 결함 편집을 잃으면 되돌린 것이
  아닙니다. 주인 없는 sidecar 는 아무도 읽지 않는 파일일 뿐이지만, 잃은 수리는 사라진 작업입니다.

**비교·살펴보기**도 붙었습니다. 둘 다 격자와 같은 자리를 씁니다 — 따로 창을 띄우면 보는 것과
고른 것이 어긋나 깃발이 엉뚱한 사진에 걸립니다. 올라가는 차례는 **격자에 보이는 차례**이며 고른
차례가 아닙니다. 후보는 활성 사진, 기준은 그 앞의 첫 사진이라 활성만 바꿔 가며 같은 기준과
견줄 수 있고, 한 사진이 자기 자신과 짝지어지는 일은 없습니다.

## 2026-08-15 타깃 고르개도 없었습니다

**현상 타깃 일곱 가지를 고를 방법이 없었습니다.** 스캐너 프로파일과 같은 모양입니다 —
`develop_export.cpp` 는 처음부터 타깃별 그레이드를 돌리고 있었고, 거기에 닿는 UI 가 없었습니다.

macOS 는 이 컨트롤들을 라이브러리 사이드바의 **현상** 구획(가져오기·스캔 아래)에 둡니다. 같은
자리에 같은 차례로 넣었습니다 — 프로세스, 타깃 다섯 칸, 그리고 필름 프로파일과 룩. 필름 프로파일
칸은 갈래에 따라 **고르는 것이 바뀝니다**: MAIN 갈래에서는 MAIN/PRINT/EXPIRED, HS·SP 에서는 그
기종의 필름 프로파일, F135·HR 에서는 고를 것이 없어 이름만.

타깃을 바꾸면 스캐너 프로파일을 뗍니다(macOS 와 같음). 미니랩 재현 타깃에서는 타깃 자체가 성격을
정하므로 프로파일을 얹으면 성격이 두 겹이 되고, MAIN·PRINT·EXPIRED 에는 맞는 프로파일이 정의상
없습니다 — 프로파일은 기종에 매이는데 그 셋은 기종이 아닙니다.

실제 V700 컬러 네거티브를 셸의 요청 경로로 일곱 번 내보내 재었습니다.
...
**뒤의 둘이 0% 인 것은 정상입니다.** EXPIRED 는 되살릴 색조 치우침을 찾았을 때만 손대고(건강한
스캔에서는 아무것도 하지 않습니다), PRINT 의 차이는 프린터 출력 프로파일에서 나오는데 그것은
인화 화면과 함께 옵니다. 지금 상태에서 PRINT 를 골라도 그림이 그대로인 것은 **아직 인화가
없기 때문**이며, 이 표에 그대로 적어 둡니다.

## 2026-08-15 스택·가상 사본·원본 옮기기

라이브러리의 남은 세 가지를 붙였습니다. 셋 다 카탈로그 쪽 일이 먼저였습니다.

**스택.** `stacks` 표는 처음부터 스키마에 있었고 읽는 사람이 없었습니다. 기능을 지탱하는 규칙은
**두 장 미만인 스택은 존재하지 않는다** 입니다 — 한 장짜리는 접어도 아무것도 감추지 않으면서
배지만 남아 고장으로 보입니다. 그래서 만들 때 두 장 미만을 거절하고, 사진을 빼면 스택이 함께
사라집니다. 대표는 **화면 차례에서 가장 앞선** 구성원이며 기록의 첫 id 가 아닙니다 — 별점순으로
바꾸면 대표도 따라가야 합니다.

**가상 사본.** payload 를 통째로 복제하고 신원 세 칸만 덮어씁니다. 아는 field 만 옮기면 새
카탈로그가 담은 값이 사본에서 빠져 두 사진의 현상 결과가 갈립니다. 함정은 결함 sidecar 였습니다 —
`hasDefectEdits` 가 payload 와 함께 복제되어 오는데 sidecar 는 원본 id 로 묶여 있어, 그대로 두면
투영이 사본을 읽지 못해 **격자에서 사라집니다.** 사본 몫의 sidecar 를 자기 id 로 새로 쓰고,
그것이 실패하면 주장 자체를 지웁니다.

**원본 옮기기.** 이 앱이 사용자의 파일을 실제로 건드리는 몇 안 되는 자리입니다. 파일이 먼저,
카탈로그가 나중입니다 — 뒤집으면 이동이 실패했을 때 카탈로그가 빈 자리를 가리킵니다. 되돌리기는
**뒤에서부터** 합니다: 앞 파일이 비운 자리를 뒤 파일이 차지했을 수 있습니다. 되돌리기까지 실패한
파일은 목록으로 알립니다. IR 짝은 본 스캔과 함께 움직입니다 — 남겨 두면 다음 검출이 다른 폴더를
봅니다.

## 2026-08-15 단축키가 생겼습니다

macOS 는 57개 명령을 사용자가 바꿀 수 있는 단축키로 냅니다. Windows 에는 **하나도 없었습니다** —
컨트롤 안의 화살표 키가 전부였습니다. 롤 하나를 훑으려면 P·X·U·0~5 가 필요하고, 그 키들은
깃발 쓰기가 붙기 전까지는 닿을 백엔드조차 없었습니다.

넣은 것은 **Windows 가 실제로 할 수 있는 37개**이며 이름과 키는 macOS 와 같습니다. 나머지는
넣지 않았습니다 — 아무 일도 하지 않는 명령을 목록에 올리면 사용자는 듣지 않는 키를 배웁니다.

**키 두 무리를 옮겨야 했습니다.** macOS 는 command 와 control 을 따로 씁니다. Windows 에는
command 가 없어 그 자리를 Control 이 가져가므로, macOS 의 `control+shift` 무리(프로세스 전환
넷)를 `Alt+Shift` 로 옮겼습니다. 겹친 채로 두면 각 쌍 중 하나는 영영 실행되지 않고 아무도
그것을 알려 주지 않습니다.

규칙은 셋입니다. 이미 쓰이는 키는 거절합니다. 빈 키로 명령을 잠그지 못합니다. 덮어쓰기가 남의
기본값을 빼앗으면 **덮어쓴 쪽이 이기고** 빼앗긴 쪽은 빈칸으로 보입니다 — 안 듣는 키를 자기
것처럼 보여 주는 것보다 낫습니다.

`WorkflowShortcutMap` 에는 값 비교를 직접 넣었습니다. record 의 기본 비교는 사전을 참조로 보기
때문에, 그대로 두면 설정 저장소가 관계없는 변경마다 파일을 다시 썼을 것입니다.

## 2026-08-15 고르개가 없어 닿지 않던 현상 단계

**스캐너 프로파일 고르개가 없었습니다.** 엔진은 NORITSU·SP-3000 프로파일 15종을 처음부터
컴파일해 들고 있었고 `develop_export.cpp` 는 프레임이 프로파일을 지목하면 그 단계를 돌립니다.
지목할 방법이 Windows UI 에 없었을 뿐입니다 — macOS Base 구획은 고르개가 셋인데 Windows 는
둘이었습니다.

실제 V700 컬러 네거티브로 셸의 요청 경로를 그대로 태워 재어 봤습니다.

| | 값 |
| --- | ---: |
| 달라진 표본 | 15,413,350 / 15,426,734 (**99.913%**) |
| 최대 차이 | 21,253 / 65535 |

미묘한 누락이 아니었습니다. 고르개는 엔진이 모르는 id 를 거부합니다 — 저장은 됐는데 그림이
그대로인 상태가 여기서 나올 수 있는 가장 나쁜 결과입니다.

덕분에 라이브러리의 마지막 빠른 필터 축에도 근거가 생겼습니다. "미검증 프로파일" 은 macOS 의
`scannerProfileState(isAnyOf: missing, draft, realOnly, pairedSmoke)` 이며 그 목록에 `none` 이
**없다**는 점이 중요합니다 — 프로파일을 고르지 않은 사진은 검증할 대상 자체가 없어 걸리지
않습니다.

## 2026-08-15 사진에 이름과 깃발과 메뉴가 생겼습니다

Windows 는 모든 사진을 파일 이름으로 불렀습니다. macOS 는 스캐너 프레임을 "사진 3" 이라
부르며, 그 번호는 파일 이름의 `_frame_<n>`, 롤 순번, 또는 사용자가 지정한 번호에서 옵니다.
macOS 에서 이름을 바꾼 사진은 그 번호를 `customDisplayName` 안에
`negaflow:photo-number:3` 으로 넣어 두는데, Windows 는 **그 문자열을 카드에 그대로 찍고
있었습니다.**

세 가지가 뒤이어 없었습니다.

- **깃발은 읽기만 되고 쓰기가 없었습니다.** 빠른 필터에 "선택" 이 있는데 무엇도 선택할 수
  없으니 그 필터는 영원히 비어 있었습니다.
- **라이브러리에서 빼기가 아예 없었습니다.** 프레임 행만 지우면 롤과 묶음에 죽은 id 가
  남습니다. 결함 sidecar 는 저장 **뒤**에 지웁니다 — sidecar 저장소는 카탈로그가 아직 그
  프레임의 결함 편집을 선언하는 동안에는 삭제를 거부합니다.
- **격자에 오른쪽 단추 메뉴가 없어** 위의 어느 것도 손에 닿지 않았습니다.

제거는 확인을 묻습니다. macOS 는 묻지 않는데, macOS 에는 실행 취소 관리자가 있고 이 빌드에는
아직 없기 때문입니다. 실행 취소가 붙으면 이 물음은 없어져야 합니다.

내보내기 `{name}` 토큰도 같은 뿌리였습니다. macOS 는 카드 이름을 넣는데 Windows 는 원본 파일
이름을 넣고 있어, 같은 사진을 두 앱에서 내보내면 다른 파일이 나왔습니다.
