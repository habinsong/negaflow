...
| 영역 | 대략 | 근거와 남은 것 |
| --- | --- | --- |
| 현상 엔진·수학 | **96%** | 반전·base(auto/manual/preset)·톤·포인트 커브·컬러 믹서·컬러 그레이딩·캘리브레이션·색·질감·FilmScanDenoise·필름 42종·GrainMend 수리·검출(자동·가이드 ROI·미세 입자)·흑백 토닝·소프트 프루프·기하 변형·긴 변 Lanczos3·출력 선명도·PNG/TIFF/JPEG 게시·TIFF LZW/Deflate·PNG/TIFF 8-bit 과 macOS 출력 디더·출력 색공간(sRGB/Display P3/Adobe RGB, ICC 생성). **8100 은 macOS 와 여섯 지점이 정확히 일치합니다**(R min 58, G min 71, B max 62256, B median 22818 대 22817). V700 은 G +0.2 · B −1.4 · **R +2.9 레벨**(8비트)까지 좁혔습니다. **남은 R 잔차의 자리를 측정으로 지목했습니다 — `CIVibrance` 입니다.** base 는 맥이 잰 값을 대입해 배제했고(+16/+762), 반전 직후까지 R 차이는 16비트 1레벨 미만이며, vibrance 를 끄고 재니 그 단계가 잔차와 같은 크기였습니다. 맥 격자로 이 필터가 화소마다 정확히 아핀이고(잔차 7.5e-8) 앵커가 Rec.709 휘도가 아니라 **산술 평균**임을 확인했습니다. 배율은 색상까지 얽혀 수식으로 되짚지 않으므로 잰 사상을 표로 넣습니다 — 17³ 표만으로도 실제 프레임 RMS 가 707 → 137(16비트)로 줍니다. **33³ 격자 대기 중**(`docs/verification/macos-golden/REQUEST-4.md`). 남은 것: alpha, 색역 클리핑(디스플레이 경계 적용 지점 대조 미완), CIVibrance 표. `docs/verification/2026-08-16-macos-pixel-golden.md` |
