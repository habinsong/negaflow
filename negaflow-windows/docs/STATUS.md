...
| 스캐너 | **96%** | 플러그인 클라이언트·발견·프로세스 호스트·프로토콜 v2·artifact 트랜잭션·게시 영수증에 더해 `ScanSessionController`·`ScannerPluginTrustStore`·`ScanStorageLayout` 이 붙어 라이브러리 가져오기 탭에서 검출→capability→스캔→카탈로그 게시가 이어집니다. 시뮬레이터로 검출→스캔→게시 전 구간을 하드웨어 없이 돌립니다. 평판 프레임 규격·프레임 자리·프레임별 스캔 영역도 붙었습니다. 프리뷰 픽셀 캡처와 자동 프레임 찾기도 UI 에서 돕니다. 합성 스트립에서 검출기가 다섯 컷을 정확한 치수로 찾는 것까지 확인했습니다. **실기 검증 완료 — OpticFilm 8100 과 Epson V700 두 대가 detect→capabilities→scan 전 구간을 돕니다.** 플러그인이 SANE 기기표에서 생성한 332종 INF 로 자립하며 벤더 소프트웨어에 기대지 않습니다. 실제 필름 프리뷰 10장에서 적재량과 검출 컷 수가 일치합니다. **적외선까지 실기 확인** — V700 이 별도 IR 채널을 냅니다. 남은 것: 실제 필름에서 IR 이 결함을 짚는지, 검출 사각형이 실제 컷 자리와 맞는지 |
