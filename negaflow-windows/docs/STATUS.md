...
**아직 확인하지 못한 것: 캔버스에서 실제로 그어 보는 것.** 자동화로 띄운 실행에서 미리보기
비트맵이 그려지지 않아(`largeImages: NONE`) 포인터 상호작용 자체가 성립하지 않았습니다.
크롭도 같은 조건에서는 동작할 수 없습니다. 이것이 자동화 환경 때문인지 제품 문제인지가
다음에 먼저 볼 것입니다 — 미리보기가 안 나오는 것이라면 그쪽이 훨씬 큰 문제입니다.
