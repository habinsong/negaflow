...
| 영역 | 대략 | 근거와 남은 것 |
| --- | --- | --- |
| 현상 엔진·수학 | **95%** | 반전·base(auto/manual/preset)·톤·포인트 커브·컬러 믹서·컬러 그레이딩·캘리브레이션·색·질감·FilmScanDenoise·필름 42종·GrainMend 수리·검출(자동·가이드 ROI·미세 입자)·흑백 토닝·소프트 프루프·기하 변형·긴 변 Lanczos3·출력 선명도·PNG/TIFF/JPEG 게시·TIFF LZW/Deflate·PNG/TIFF 8-bit 과 macOS 출력 디더·출력 색공간(sRGB/Display P3/Adobe RGB, ICC 생성). 남은 것: alpha, 색역 클리핑, **macOS 픽셀 대조 — R 채널 +975**. 장면 통계 표본을 면적 평균으로 고치고 표본 격자를 macOS 와 맞춰 G·B 차이가 열 배 줄었고(G max 는 60416 으로 정확히 일치), R 만 반대쪽에 남았습니다. base 와 어두운 비필름 게이트는 둘 다 원인이 아님을 측정으로 확인했습니다. **8100 은 macOS 와 사실상 일치합니다** — median 19 → 25161(macOS 25353), B mean 차이 5.6, G max 정확히 일치. base 추정기 표본 격자까지 면적 평균으로 고친 결과입니다. `docs/verification/2026-08-16-macos-pixel-golden.md` |
...
**전체 어림 98%** 입니다. 인화를 포함한 값이며, 확인하지 않은 것(순수 ARM64 실행,
macOS 픽셀 대조)은 완료로 세지 않았습니다 — 둘 다 이 기계에 없는 하드웨어가 필요합니다.
