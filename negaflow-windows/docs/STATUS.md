...
| 내보내기 | **62%** | PNG16·TIFF16·JPEG8 을 폴더·파일명 패턴(순번 포함)으로. 출력 패널의 파일·품질·소스 하위 탭에서 DPI·긴 변·TIFF 압축·JPEG 품질·출력 선명도·선명도 매체를 고르며, 고른 값이 실제 요청에 실립니다. 빠른 내보내기 절도 macOS 기본값으로 동작합니다. 세 포맷은 WIC sRGB ICC·DPI·구조/픽셀 readback 과 비덮어쓰기 게시를 거칩니다. 남은 것: 출력 색공간, PNG/TIFF 8-bit·alpha, 내보내기 설정 프리셋, 메타데이터 정책, 배치, XMP sidecar |
