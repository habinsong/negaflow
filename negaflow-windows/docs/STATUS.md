...
| 라이브러리 화면 | **82%** | 카드 격자·정렬·카드 크기·소스 막대·파일 트리·폴더 프로세스 선택기·별점·깃발·가져오기·relink·중복 후보. 필터는 macOS 9개 중 7개(메타데이터 상태 미확인 추가). 컬렉션(수동)도 만들고 고르고 지웁니다. 남은 것: 현재 롤·미검증 프로파일 필터, 스마트 컬렉션·저장된 검색 |
