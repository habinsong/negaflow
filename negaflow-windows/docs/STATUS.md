...
| 인화 | **83%** | 용지 27종·방향·여백·해상도·천공(ISO 1007 135)·공정 겉모습(시아노타입·건판·젤라틴실버)·컨택트 시트(행·열·간격·맞추기/채우기·돌려 맞추기·한 판 한 사진·바탕색)·**픽처 패키지 템플릿 3종**·**캡션**·**재단선**·눈금자 설정까지 macOS 와 **같은 수**로 계산합니다. 검사기와 캔버스 미리보기가 붙었고, 판을 PNG 로 씁니다(해상도 태그 포함). 실촬영 확인 완료. 남은 것: 커스텀 패키지의 자유 배치, 캡션 글자 그리기(자리는 잡습니다), 눈금자 눈금 그리기 |
| ARM64·CI | **50%** | CI 에 native·managed·arm64 cross 잡이 있습니다. **순수 ARM64 실행은 하드웨어가 없어 확인 못 함** |

**전체 어림 91%** 입니다. 인화를 포함한 값이며, 확인하지 않은 것(순수 ARM64 실행,
