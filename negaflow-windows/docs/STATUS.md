...
| 라이브러리 화면 | **94%** | 카드 격자·정렬·카드 크기·소스 막대·파일 트리·폴더 프로세스 선택기·별점·깃발·가져오기·relink·중복 후보·다중 선택. **필터 9개 전부**를 macOS 차례로 냅니다. 수동 컬렉션을 만들고 고르고 지웁니다. 스마트 컬렉션·저장된 검색도 만들고 고르고 지웁니다. **카드 오른쪽 단추 메뉴**가 macOS 차례로 붙었습니다 — 현상·이름 변경·별점·선택·제외·컬렉션 추가/제거·탐색기에서 보기·제거. 사진 이름도 macOS 규칙(사진 번호·`_frame_<n>`·롤 순번)을 따릅니다. 남은 것: 원본 파일을 폴더 사이로 끌어 옮기기, 가상 사본, 스택 |
...
**전체 어림 76%**, 사용자가 우선하라고 한 **라이브러리+현상 범위로는 약 89%** 입니다.
