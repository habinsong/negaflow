...
| GPU | **2026-08-18 착수.** 화소별 커널 **11개** + 이웃 원시연산 **4개**(박스·가우시안·중앙값·가이드필터), 전부 CPU 동치 `1e-5` 고정. **박스·가우시안·중앙값은 비트 단위 일치.** **톤 단계 7/7 완주** + `filmScanShrink` 전 사슬(실측 1.2e-07, CPU 와 같은 타일로). 파이프라인 미연결·속도 미측정·내장 GPU 실기 미확인 | [`04`](04-gpu-plan.md) 0절 · [`13`](13-performance-playbook.md) |
