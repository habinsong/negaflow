...
| [`04-gpu-plan.md`](04-gpu-plan.md) | **GPU 이식 계획 + 진행 상황.** **0절이 현재 상태입니다** — 이식 완료 커널 10개와 실측 오차, 남은 것이 왜 막혔는지, 아직 아닌 것 4가지, 시험이 잡은 실제 버그 3개. macOS `[[stitchable]]` 커널 32개 중 3개는 **macOS 가 부르지 않고** 7개는 **Windows CPU 판부터 없습니다** |
...
| GPU | **2026-08-18 착수.** 커널 **10개 + 박스 블러**, 전부 CPU 동치 1e-5 고정(노출·원색보정·박스블러는 **비트 단위 일치**). **톤 단계 7/7 완주.** 파이프라인 미연결·속도 미측정·내장 GPU 실기 미확인 | [`04`](04-gpu-plan.md) 0절 · [`13`](13-performance-playbook.md) |
