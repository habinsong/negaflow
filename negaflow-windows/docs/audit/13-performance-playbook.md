...
## 18. 밴드 측정을 GPU 로 — 원시연산은 섰습니다 (배선은 아직)

17절이 범인을 `measure_parametric_tone_curve_bands` 의 **전 화소 CPU 패스 두 번**으로
확정했습니다. 그중 하나(`downsample_for_statistics` 의 밉맵 축소)를 GPU 로 옮길 원시연산을
만들고 **비트 단위 일치를 증명**했습니다.

### `GpuMipHalve` — 비트 단위 일치
...
**허용 오차가 아니라 완전 일치를 봅니다.** 이 값이 밴드 백분위로 가므로 마지막 비트가
달라지면 밴드가 달라지고 **출력 화소가 달라지기** 때문입니다.

비트 단위가 가능한 이유: `halve` 는 float32 덧셈 셋과 `* 0.25` 뿐입니다. 곱셈이 2의
거듭제곱이라 반올림이 없고, 덧셈 순서만 CPU 와 같게 두면 값이 같습니다(`precise` 로 고정).

### ☠️ 최종 이중선형은 옮기지 않았습니다 — 옮길 수 없습니다

CPU 판 `bilinear`(`mipmap_downsampler.cpp:49`)은 가중치와 누적을 **`double`** 로 합니다.
**D3D11 의 double 은 선택 기능**이라 벤더에 따라 없습니다 — 비트 단위로 옮길 수 없습니다.

그래서 나누는 지점이 이렇습니다:

| 무엇 | 어디서 | 왜 |
|---|---|---|
| 밉맵 축소(전 화소를 훑는 부분) | **GPU** | float32 뿐이라 비트 단위 가능 |
| 마지막 이중선형 | **CPU 그대로** | `double` 이라 옮길 수 없고, 그때 화소는 이미 작아서 쌉니다 |

### 아직 안 한 것 — 배선

원시연산은 섰지만 `measure_parametric_tone_curve_bands` 가 아직 이것을 쓰지 않습니다.
쓰려면 `imaging` 쪽에 **이미 축소된 단계를 받는 진입점**이 필요합니다
(`downsample_for_statistics` 가 축소와 보간을 한 함수에 묶어 두었습니다).

그리고 나머지 한 패스인 `validate_finite_pixels` 는 그대로 남습니다.
GPU 리덕션으로 옮길 수 있지만 **실패를 화소 단위로 올리려면 UAV 카운터**가 필요합니다 —
`negative_invert.hlsl` 헤더에 적힌 것과 같은 제약입니다.

**즉 17절의 140 ms 중 얼마가 줄지는 배선한 뒤에 재야 압니다. 지금은 모릅니다.**
