...
#### 남은 창까지 닫았습니다 (2026-08-20)

통지를 잠금 안으로 옮기는 것만으로는 **다른 스레드가 막 풀어 준 뮤텍스를 곧바로
없애는** 창이 원리적으로 남습니다(대기자가 깨어 돌아가며 `PendingCounter` 를 파괴하는
사이 워커의 `lock_guard` 소멸자가 아직 그 뮤텍스를 만지는 경우). 그래서 계수기의
모양을 바꿨습니다.

| | 전 | 후 |
|---|---|---|
| 호출부 계수기 | `std::mutex` + `std::condition_variable` + `uint32_t` | **`std::atomic<uint32_t>` 하나** |
| 워커가 계수기를 만지는 마지막 순간 | `lock → --remaining → notify → unlock` | **`fetch_sub` 하나** |
| 기다림에 쓰는 뮤텍스·조건변수 | 호출부 스택(사라질 수 있음) | **풀 소유**(프로세스 수명, 일부러 안 지웁니다) |
| 대기 | 계수기의 조건변수 | 풀의 조건변수 + 술어는 **자기 원자값**만 읽음 |

이제 워커는 `fetch_sub` 뒤로 호출부 객체를 **두 번 다시 만지지 않고**, 대기자는 그 값이
0 이 된 것을 본 뒤에만 돌아옵니다 — 사라진 객체를 만질 수 있는 순서가 없습니다.

**성능 A/B (같은 기계·같은 원본, `--develop-timing` 6회씩, 중앙값):**

| | TOTAL |
|---|---:|
| 전(뮤텍스 계수기) | **408.24 ms** (401.5 ~ 446.0) |
| 후(원자 계수기) | **409.01 ms** (395.7 ~ 432.6) |

차이 0.8 ms(0.2%)로 실행 간 흔들림(±25 ms)보다 훨씬 작습니다 — **되돌릴 이유 없음.**

**재시험:** ctest 102/102 · `gpu_film_scan --repeat until-fail:60` 60/60 ·
동시 프로세스 6개 × 12바퀴 72회 · ci-gate catalog 747 · shell 1411 · 경고 0.
