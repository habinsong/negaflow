...
# 14 — GPU 방법론 — **할 일 목록이 아님. 2026-08-19 이후 열지 말고 15를 보십시오**

**작성 2026-08-18.** 닫힌 이식과 남은 확인은 [`15`](15-gpu-handoff.md)입니다.
2·3·6·7.2절은 **무효**입니다 — 죽은 커널을 이식하라고 적혀 있습니다.
오판 정정은 0.0절과 [`06`](06-false-claims.md) 10–13절. 타일·초월함수 제약만 참고합니다.

 **이 문서를 쓰면서 제 앞 판정 세 개가 틀린 것을 찾았습니다.** 0절에 먼저 적습니다.

---

## 0.0 2026-08-19 — **이 문서의 0.1·0.2·2·3·7.2 절이 틀렸습니다**
...
## 0. 먼저 — [`04`](04-gpu-plan.md) 0.3절의 오류 세 개
 ...
 **이것은 창작이 아니라 한계의 기록입니다.** Apple 의 수열을 알 수 없으므로
...
3. **`scaled_noise` 의 `size > 1.01` 경로에 `double` 이 있습니다**(`:53-56`).
...
## 2. ~~`digitalFilmColor`~~ — **무효.** 그 커널은 macOS 에서 죽었습니다 (0.0절)

### 먼저 확인된 발산 — 이것부터 판단이 필요합니다
...
> **이것은 GPU 문제가 아니라 이식 정확성 문제입니다.** GPU 로 옮기기 전에
...
2. CPU 골든을 세운 뒤에 GPU.
3. **`digitalHueBand` 의 `float anchors[6]` 은 HLSL 에서 그대로 쓰지 마십시오.**
...
3. **`domainWeight` 는 sRGB 코드 좌표에서 계산합니다**(주석이 명시). working-linear 에서
 ...
 **적용 도메인을 틀리면 안 됩니다.** 선형 광에서 `1/255` 를 더하면 암부에서 수십 배로 보입니다.
 ...
 **경계값 정의를 그대로 옮기십시오.** `<= 0.0` 과 `>= 1.0` 입니다 — 주석이
*"경계에 정확히 있는 채널은 정의상 클리핑"* 이라고 못박습니다. `< 0` / `> 1` 로 바꾸면
정확히 0/1 인 화소가 경고에서 빠집니다.

 **프리멀티플라이드 나눗셈을 빠뜨리지 마십시오.** Windows 작업 이미지가 프리멀티가
아니라면 그 단계는 **빼야** 합니다 — 옮기기 전에 어느 쪽인지 확인하십시오.

---

## 6. ~~CPU 판부터 없는 일곱~~ — **무효.** 다섯이 죽은 커널입니다 (0.0절)
...
### GPU 로 넘길 때 — 사슬이라 **한 번에 올리고 한 번에 내립니다**
 ...
 7단계를 **단계마다 업로드·다운로드하면 집니다.** GPU 형태학이 그래서 CPU 보다 느렸습니다
...
2. **재기 전에는 `NEGA_GPU_MORPHOLOGY` 를 켜지 마십시오.** 지금 켜면 느려집니다.
 ...
 **[`04`](04-gpu-plan.md) 0.5절의 타일 규칙을 그대로 옮겨야 합니다.**
 ...
 **부동소수 덧셈은 결합법칙이 성립하지 않습니다.** 리덕션 순서가 CPU 와 다르면 값이 다릅니다 —
...
## 9. 2026-08-20 — 규칙 하나 더: **시험은 재현한 뒤에 씁니다**

이 세션은 앱이 죽는 자리를 보고 **가설로 스트레스 시험을 먼저 만들었습니다.**
그 시험은 10/10 통과했고, 원인은 다른 곳(소멸 순서)이었습니다. 시간만 썼습니다.

> **먼저 재현하십시오.** 앱을 띄우고, 그 단추를 여러 번 누르고, 죽으면 기록기를 붙여
> 예외 코드·주소·스택을 잡습니다. **그 증거로 원인을 확정한 뒤에** 그 자리를 고정하는
> 시험을 씁니다. 순서를 바꾸면 시험이 **아무것도 증명하지 않습니다**.

연장은 [`01`](01-backend-gaps.md) 9.3 · [`11`](11-ui-verification-protocol.md) 6절.
