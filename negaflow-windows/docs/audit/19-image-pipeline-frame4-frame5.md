...
2. **`FilmBaseEstimator`(connected_component)를 macOS Swift 와 줄 단위로 대조** — 지금 가장 급한 일.
   맥이 더 짙은 베이스를 잡습니다. 우리 표본이 얇다는 신호(`sparseSampleCoverage`)가 있습니다.
3. 고친 뒤 `compare16` 으로 다시 재서 전달곡선 차이가 0 으로 가는지 확인.
