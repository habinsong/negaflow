...
## 지금 가장 큰 결함: 한글이 든 경로의 사진이 현상되지 않습니다

GrainMend 브러시를 실제 앱에서 그어 보려다 찾았습니다. **셸 안에서 frame 의 원본 경로가
깨집니다.** 카탈로그에는 멀쩡히 들어 있는데 앱이 들고 있는 문자열이 다릅니다.

관측한 것 그대로:

- 카탈로그: `C:\Users\habin\OneDrive\바탕 화면\negaflow_test\OpticFilm8100_frame_1.tiff`
- 앱 안: `C:\Users\habin\OneDrive\諛뷀깢 ?붾㈃\negaflow_test\...`, `exists=False`
- 그래서 현상은 `ObserveSourceBefore open_failed native=0x7B`(ERROR_INVALID_NAME)로 끝나고,
  화면에는 "이미지를 가져오세요" 빈 상태가 뜹니다. 필름스트립 썸네일도 깨진 자리표시자입니다.

깨진 문자열은 **UTF-8 바이트를 CP949 로 읽었을 때 나오는 모양**이고 중간에 물음표(0x3F)가
섞여 있습니다 — 어딘가에서 손실 있는 인코딩 변환이 일어납니다.

**이미 배제한 것들** (다시 재지 마십시오):

- 네이티브는 한글 경로를 잘 엽니다. `--probe-open "<한글 경로>"` 로 확인했고,
  probe 와 preview 모두 성공합니다(800x534). ASCII 경로와 결과가 같습니다.
- 카탈로그 파일도 멀쩡합니다. 앱을 띄우기 **전과 후** 모두 `--diagnose` 가 올바른 경로와
  `exists=True` 를 냅니다. 앱이 파일을 망가뜨리는 것이 아닙니다.
- SQLite provider 차이도 아닙니다. 시험 실행기에 셸과 같은 `e_sqlite3.dll` 을 넣고 다시 읽어도
  경로는 올바릅니다.
- frame id 형식, 저장소 뿌리, 선택 순서(라이브러리 먼저 / 현상 먼저) 모두 무관합니다.

**다음 한 걸음:** 셸 프로세스 안에서 catalog payload 바이트를 읽는 순간에 덤프해 보십시오.
`SqliteCatalogStore` 가 payload 를 byte[] 로 읽어 `JsonNode.Parse` 에 넘기는 자리와,
`LibraryFrameReader` 가 `rawScanPath` 를 꺼내는 자리 둘 중 어디에서 이미 깨져 있는지가
그 한 번으로 갈립니다. 같은 코드가 콘솔 프로세스에서는 멀쩡하므로, WinUI 프로세스에만 있는
조건(전역화 설정, app.manifest 에 `activeCodePage` 가 없다는 점)을 함께 보십시오.

재현: `Negaflow.Shell.UnitTests.exe --seed <root> "<한글이 든 경로의 TIFF>"` 로 심고
`NEGAFLOW_STORAGE_ROOT=<root>` 로 셸을 띄운 뒤 현상 화면에서 사진을 고릅니다. 실패하면
캔버스 아래 상태줄에 이유가 나옵니다(이번에 붙였습니다).

**이 결함이 다른 무엇보다 먼저입니다.** 한국어 사용자의 스캔은 대개 `바탕 화면`, `사진`,
`문서` 아래에 있고, 그 경우 현상 화면이 아무것도 못 엽니다.
