...
sidecar 영속 lifecycle, preview/export 공통 request 투영은 완료했습니다. 색상 네거티브와 색상 슬라이드는
허용하고 silver image가 IR을 막는 흑백 네거티브·포지티브는 거부합니다. 다음 체크포인트는 scanner
companion plane 수집입니다. 그 뒤 같은 입력의 macOS-hosted
...
Film Emulation의 다음 hosted golden도 남아 있지만, **현재 다음 한 걸음은 scanner companion plane을
ABI 0.33 검출 경로에 공급하는 것**입니다. 수치 golden은 실제 화면 결과 차이를 잡는
