...
**색상 섹션이 없었습니다.** 색온도·틴트·생동감·채도·색 깊이 다섯 축이 catalog·엔진까지는
연결돼 있었는데 컨트롤이 없어 자동 화이트 밸런스 말고는 닿을 길이 없었습니다. macOS 순서대로
톤 곡선과 컬러 믹서 사이에 넣었습니다. **아직 없는 섹션: 흑백 토닝** — Interop 에는 있고
catalog recipe 에는 없습니다. 그게 남은 인스펙터 공백입니다.

**검증용 씨앗:** `Negaflow.Shell.UnitTests.exe --seed <root> <tiff>...` 가 실제 스캔을 가리키는
카탈로그를 만듭니다. 인스펙터는 프레임이 있어야 UIA 트리에 나오므로, 오른쪽 패널을 확인하려면
먼저 이걸 쓰십시오. 가져오기 대화상자를 자동화하는 것보다 훨씬 쌉니다.
