...
**다음에 가장 먼저 볼 것: 씨앗으로 넣은 프레임이 현상 화면에 안 뜹니다.** GrainMend 브러시를
실제 앱에서 그어 보려다 막힌 자리이며, 여기까지 좁혀 두었습니다.

- **카탈로그는 멀쩡합니다.** `Negaflow.Shell.UnitTests.exe --diagnose <root>` 로 셸과 같은
  방식으로 열어 보면 `state: Open`, `frames: 1`, `exists=True`, `metadata=present`,
  `request=ok` 입니다 — 즉 그 프레임으로 현상 요청이 정상적으로 만들어집니다.
- **그런데 화면에는 "이미지를 가져오세요" 빈 상태가 뜹니다.** 현상 왼쪽 머리줄은 "사진 없음"
  이고, 필름스트립의 썸네일은 깨진 이미지 자리표시자입니다. 라이브러리 격자에서 먼저 고르든
  현상으로 옮긴 뒤 필름스트립에서 고르든 같습니다. frame id 를 GUID 로 바꿔도 같았습니다.
- 같은 조건에서 **인스펙터는 정상**입니다 — 색온도를 바꾸면 저장되고 다시 띄워도 남습니다.
  그러니 패널 상태·카탈로그·현상 요청은 문제가 아니고, **선택이 현상 뷰까지 전달되지 않거나
  썸네일/미리보기 렌더가 실패하는 것** 둘 중 하나입니다.

**사람이 한 번 직접 띄워서 사진을 눌러 보는 것이 가장 빠릅니다.** 보이면 자동화 쪽 한계이고,
안 보이면 그것이 지금 가장 큰 문제입니다(사진이 안 보이는 현상 화면은 쓸 수 없습니다).
가져오기로 넣은 프레임과 씨앗으로 넣은 프레임을 비교해 보면 씨앗 record 에 빠진 필드가
있는지도 함께 드러납니다.
