...
**다음 한 걸음은 사람 손으로 하는 확인 하나입니다.** 라이브러리 폴더 머리줄에서 Digital Color
를 고른 뒤, 현상 → 필름 탭을 열어 42종 목록이 그려지고 하나를 고르면 캔버스가 바뀌는지 보십시오.
백엔드는 단위 시험이 증명했고(462 assertion) 선택기에 여섯 항목이 나오는 것도 확인했지만,
자동화로는 팝업에서 항목을 고르지 못해 **적용되는 순간을 못 봤습니다.** 이것이 지금 가장
싸고 가장 값진 확인입니다.
