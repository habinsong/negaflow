...
**스캐너도 닿지 않습니다.** `Shell.Core/Scanner` 에 플러그인 클라이언트·발견·프로세스 호스트·
프로토콜·artifact 트랜잭션·게시 영수증이 다 있는데 `src/Shell` 어디에서도 부르지 않습니다.
이번 세션에 같은 모양으로 찾은 것이 다섯 번째입니다(프리셋, 색상 다섯 축, 흑백 토닝,
자동 GrainMend 세기, 스캐너). **새 기능을 만들기 전에 이 패턴부터 한 번 훑으십시오** —
`Shell.Core`/`Interop`/native 에 있는데 `src/Shell` 에서 참조가 0인 공개 API 를 찾으면 됩니다.
