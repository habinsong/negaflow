...
고정 요구: 다국어 텍스트가 길어져도 UI가 잘리지 않게 할 것. Library/Develop/Print는 하나의 워크플로. `main`에서만 작업. 로컬 CI로 확인하고 GitHub Actions 실패 check와 원본 로그를 본다. 미검증 항목은 완료로 쓰지 않는다.

> **2026-08-16 이력 재개 지점:** 이 문서의 아래 내용은 2026-08-13 당시의 이력입니다.
...
## 2026-08-13 현재 재개 지점 — 라이브러리·현상 제품 표면

방향이 바뀌었습니다. 백엔드(M2/M4/M6 는 94~97%)가 아니라 **제품 표면이 남은 큰 덩어리**이고,
...
카드에서 현상으로 넘어가기, 라이브러리 소스 막대와 파일 트리, Develop 편집 탭의 종횡비·잠금·
각도 다이얼, 자동 색상/자동 레벨 토글, 현상 인스펙터 다국어화. 상세는 `../STATUS.md` 2026-08-13.

아직 남은 하드코딩 영어 문자열은 **설정과 인화 화면**입니다(약 20곳). 현상 경로는 끝났습니다.

프로세스 선택 → route 변경 → 필름 목록 → 캔버스 변화까지 **화면에서 끝까지 확인했습니다**
(`../STATUS.md` 참조).

**대량 라이브러리도 실측했습니다**(`../verification/2026-08-13-library-scale.md`). 가상화는
정상이고(1,500장에서 370개 realize), 재활용 카드의 비트맵 누수를 잡아 1,270 → 861 MB 로
줄였습니다.

메모리 조사는 **끝났습니다.** `dotnet-gcdump` 로 확인한 결과 1,500장 warm 864 MB 중 관리
힙은 **36 MB(4%)** 뿐이고, 나머지는 WinUI 네이티브·컴포지션 표면입니다. **관리 누수는
없으므로 그쪽을 더 파지 마십시오.** 가설 셋을 재서 전부 기각한 표와 함께
`../verification/2026-08-13-library-scale.md` 에 있습니다. cold(약 1,255 MB)와
warm(약 858 MB)을 섞어 비교하지 마십시오.

**UI Automation 요령 하나:** ComboBox 팝업은 **같은 PowerShell 프로세스 안에서** 열고 골라야
...
**현상 편집이 저장되지 않고 있었습니다.** 지금은 macOS 와 같은 1.5 초 지연 저장과 닫을 때의
마지막 쓰기가 붙었습니다. 예약은 `LibraryHostService` 의 편집 세 함수 안에서 겁니다 —
호출부에 맡기면 한 군데만 빠져도 그 편집이 사라집니다. **UI 를 새로 붙일 때 이 자리를 우회해
`LibraryDocument` 를 직접 만지지 마십시오.**

**인스펙터 섹션이 둘 비어 있었고 둘 다 채웠습니다.**

- **색상** — 색온도·틴트·생동감·채도·색 깊이. catalog·엔진까지는 연결돼 있었는데 컨트롤이
  없어 자동 화이트 밸런스 말고는 닿을 길이 없었습니다. 톤 곡선과 컬러 믹서 사이입니다.
- **흑백 토닝** — 네이티브는 진작 있었고 catalog recipe 가 없었습니다. 흑백 필름에서만 나오며,
  색조 기본값과 켤 때의 최소 세기 0.45 가 macOS 와 같습니다.

**이제 macOS 인스펙터의 여덟 섹션이 모두 있습니다.** 기본 톤·톤 곡선·색상·컬러 믹서·컬러
그레이딩·흑백 토닝·캘리브레이션·디테일 및 효과. 정보 탭도 채웠습니다.

## 검증 함정: PowerShell 5.1 은 BOM 없는 .ps1 을 CP949 로 읽습니다

한동안 "한글 경로의 사진이 현상되지 않는다"고 적어 두었던 자리입니다. **제품 문제가
아니었습니다 — 검증 스크립트가 깨진 경로를 심고 있었습니다.**

무슨 일이었냐면: 검증용 `.ps1` 을 BOM 없는 UTF-8 로 저장했는데, Windows PowerShell 5.1 은
BOM 이 없으면 스크립트를 ANSI(여기서는 CP949)로 읽습니다. 그래서 스크립트 안의
`바탕 화면` 이 `諛뷀깢 ?붾㈃` 이 된 채 씨앗에 들어갔고, 앱은 **있지도 않은 파일을 정확히
그대로 보고**했습니다(`ObserveSourceBefore open_failed native=0x7B`). 도구를 인라인으로
실행할 때는 인자가 UTF-16 으로 전달되어 멀쩡했기 때문에, 같은 경로가 어떤 실행에서는 되고
어떤 실행에서는 안 되는 것처럼 보였습니다.

**검증 스크립트에 한글이 들어가면 BOM 을 붙여 저장하십시오.** 아니면 경로를 인라인 인자로
넘기십시오. 이걸 몰라서 제품에 없는 결함을 한참 쫓았습니다.

건진 것 둘: 미리보기 실패 이유를 캔버스 아래에 내게 됐고(그 전에는 "이미지를 가져오세요"
빈 화면만 나와서 현상 실패와 사진 없음이 구별되지 않았습니다), 시험 실행기에
`--diagnose`·`--probe-open` 이 생겼습니다.

## "만들어는 뒀는데 닿지 않는 기능" 훑기 — 끝냈습니다

이번 세션에 같은 모양으로 여섯 번 나왔습니다(프리셋, 색상 다섯 축, 흑백 토닝, 자동 GrainMend
세기, 스캐너, 소프트 프루프). 그래서 손으로 찾지 말고 돌리도록 만들어 두었습니다:

```
py scripts/find-unreachable-api.py
```

`Shell.Core`·`Interop` 의 공개 타입 175개 중 `src/Shell` 이 이름조차 부르지 않는 것 110개를
냅니다. **출력은 손으로 걸러야 합니다** — 배관 타입이 대부분입니다(`DevelopExportRequest` 는
`DevelopRequestFactory` 가 만들어 넘기므로 셸이 이름을 부를 이유가 없습니다).

**걸러 낸 결과, 라이브러리·현상 범위에는 이제 남은 것이 없습니다.** 진짜 후보 셋은 모두
다른 이유로 막혀 있습니다:

- **스캐너**(약 25개 타입) — 플러그인 클라이언트·발견·프로세스 호스트·프로토콜·artifact
  트랜잭션·게시 영수증이 다 있으나 UI 가 없습니다. 스캐너 하드웨어 없이는 의미 있게 확인할
  수 없고, 플러그인 구현체도 아직 없습니다.
- **소프트 프루프**(`SoftProofSettings`·`SoftProofMedia`·`SoftProofSimulation`) —
  `PreviewCoordinator.SoftProof` 가 공개 setter 인데 셸이 한 번도 넣지 않습니다. macOS 에서
  이 스위치는 **내보내기 설정**(`exportSettingsStore.softProofEnabled`)에 있으므로 자리는
...
1. **GrainMend 네 도구와 미세 입자 검출(완료).** 브러시·복제 도장·자동·가이드 ROI 검출, review→수락→sidecar
   ...
   **마스크 → catalog 변환도 만들었습니다.** `GrainMendRegionEdit.From` 이 검출 마스크를
   region 항목으로 넓힙니다. **표현이 다릅니다** — 검출기는 화소당 1바이트를 내주는데 catalog
   의 region 마스크는 RGBA8 입니다(검증기가 4바이트/화소를 요구하고 네이티브 region 편집도
   stride 를 width×4 로 받습니다). 네 채널에 같은 값을 넣습니다. 시험을 붙여 뒀고, 거기서
   ...
   **자동은 끝났습니다.** 버튼 → 검출 → 오버레이 → Enter 로 반영, Esc 로 취소까지 붙였고
   실제 스캔으로 확인했습니다: 9,187 곳을 찾아 오버레이가 뜨고, **그 상태에서는 sidecar 가
...
2. **GrainMend 브러시·복제 도장의 좌표 계약**(이미 구현했지만 손댈 때 알아야 합니다):
   ...
   걸린 프레임에서 엉뚱한 자리를 지웁니다. 역변환이 필요하고, 엔진이 적용하는 순서는
   `imaging/image_transform.cpp` 기준으로 **orient(90° 회전 + 반전) → straighten → crop**
...
**검증용 씨앗:** `Negaflow.Shell.UnitTests.exe --seed <root> [--bw] <tiff>...` 가 실제 스캔을
가리키는 카탈로그를 만듭니다. 인스펙터는 프레임이 있어야 UIA 트리에 나오므로, 오른쪽 패널을
확인하려면 먼저 이걸 쓰십시오. `--bw` 는 흑백 route 로 심어 흑백 전용 섹션을 볼 수 있게
합니다. 가져오기 대화상자를 자동화하는 것보다 훨씬 쌉니다.
