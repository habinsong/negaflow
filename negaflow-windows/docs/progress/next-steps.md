...
**다음에 가장 먼저 볼 것: 미리보기가 그려지지 않는 것처럼 보입니다.** GrainMend 브러시를
실제 앱에서 그어 보려다 발견했습니다. `--seed` 로 프레임을 넣고 현상 화면으로 옮긴 뒤에도
창 안에 큰 Image 요소가 하나도 없었고(`largeImages: NONE`), 그 상태에서는 `TryCanvasUnitPoint`
가 항상 실패하므로 브러시도 크롭도 성립하지 않습니다. **먼저 사람이 직접 띄워서 사진이
보이는지 확인하십시오.** 보인다면 자동화 환경의 문제이고, 보이지 않는다면 그것이 지금 가장
큰 문제입니다. 인스펙터 값 편집(색온도·흑백 토닝)은 같은 조건에서 정상 동작하므로 패널 상태와
카탈로그 경로는 멀쩡합니다 — 캔버스 렌더만의 문제입니다.
