...
## 5. 지금 상태 (측정된 사실만 — 2026-08-18 갱신)

### 5.0 이식 순서 (사용자 지시, 이 순서를 지킬 것)

**자동 → 가이드 → 브러시 → 복제 도장 → IR.** 한 도구의 **백엔드와 프론트엔드를 모두**
끝내고 다음으로 갑니다. 이것저것 섞지 않습니다. 이식은 macOS Swift 를 **줄 단위로 베낍니다
— 창작 금지.** 창작 때문에 이미 30회 넘게 되돌렸습니다.
...
| macOS Swift | Windows | 수치 |
|---|---|---|
| `BrushOverlay.paint` | `GrainMendPaintOverlayRenderer` | `Color.red.opacity(0.45)` = (255,59,48)@0.45, 굵기 `max(1, thickness × min(w,h))`, round cap/join, 점 하나면 그 지름의 원 |
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| frame_2 | **89**/89 | **3**/3 | 39/34 | 15/21 | 9/11 | **1**/1 | 1,526/1,561 | 1,682/1,720 |
| frame_3 | **102**/102 | **2**/2 | 28/29 | **12**/12 | 2/3 | **0**/0 | 409/475 | 555/623 |
| frame_4 | **13**/13 | **2**/2 | **5**/5 | 9/10 | **0**/0 | **0**/0 | 164/197 | 193/227 |
| frame_5 | **102**/102 | **8**/8 | 27/22 | 12/11 | **2**/2 | **0**/0 | 414/502 | 565/647 |
| frame_7 | **175**/175 | **2**/2 | 34/30 | **11**/11 | **2**/2 | **0**/0 | 684/736 | 908/956 |

**먼지·핀홀·유제는 다섯 장 전부 정확히 일치.** 남은 것: frame_2 세로 15/21(−29%, ±20%
밖), frame_5 가로 +23%, 미세입자 −7~17%.

### 5.2 검출 품질 — 가이드 (frame_7 중앙 50% ROI)

macOS 기준값을 이 세션에 Parsec 으로 **직접 재어 확보**했습니다(전에는 자동만 있었습니다).
...
가로 스크래치 수백 개는 오검출이 아니라 macOS 의 동작입니다 — 가이드는 격자·연장 배제를
끕니다. 이 기준값이 없던 동안 368 을 "가이드가 깨졌다"로 오판했습니다.
