...
//
// **가중치를 여기서 만들지 마십시오.** `imaging::prepare_film_emulation_acutance` 가
// 만든 것을 상수 버퍼로 받습니다. 두 곳에서 만들면 `exp` 구현 차이가 화소마다 실립니다.
//
// 주의 CPU 는 두 패스 모두 `double` 로 누적합니다. GPU 는 float 입니다 — 11항이라
// 누적 오차가 1e-6 대이고, 언샤프 세기(≤0.3)가 다시 눌러 출력에서는 1e-7 대입니다.
// 시험이 그 값을 고정합니다.
//
// 주의 중간 텍스처는 CPU 스크래치와 같은 **float32 RGB** 반올림을 지납니다. CPU 가
...
cbuffer FilmEmulationAcutanceConstants : register(b0) {
    uint2 Extent;
    float2 Padding0;
    // 주의 HLSL 상수 버퍼의 배열은 원소마다 16바이트입니다. 11탭을 `float4[3]` 로 묶어
