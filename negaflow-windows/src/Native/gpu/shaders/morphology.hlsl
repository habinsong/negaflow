...
//
// 주의 **지금은 창을 직접 훑습니다(화소당 O(r)).** 원래 이 선택의 근거는 "실제 반경이
// 1·3·4·8·12(창 최대 25)로 작아서" 였고, 바꾸려면 **재고 나서** 하라고 적어 두었습니다.
//
// 2026-08-25 에 쟀습니다. 두 가지가 나왔습니다.
//
// ① 그 전제는 IR 검출에서 이미 깨져 있습니다. `infrared_defect_detector.cpp:158` 의
//    반경은 `clamp(min(width,height)/100, 4, 96)` 이라 실제 스캔(2272×3431)에서 **22**,
//    창 **45** 이고 큰 스캔에서는 상한 96, 창 193 까지 갑니다. 시험 반경도 그 낡은
//    목록 그대로여서 실제 값이 하나도 안 덮여 있었습니다 — 지금은 22·64·96·97 을 덮습니다.
//
// ② **그런데 창을 넓혀도 여기가 병목이 아닙니다.** 인접 스레드의 겹치는 읽기를
//    groupshared 로 한 번만 올리도록 고쳐서 실제 부산 22쌍을 재 봤더니, 같은 빌드를 두 번
//    돌린 편차 안이었습니다(`ir_signal` p95 직접 586.7/564.3 대 타일 558.0/567.8 ms).
//    GPU 의 2D 텍스처 캐시가 슬라이딩 1D 창의 중복을 이미 흡수하고 있다는 뜻입니다.
//    근거 없는 복잡도를 남기지 않으려고 그 변경은 되돌렸습니다.
//
// 그러므로 여기를 다시 건드리려면 **읽기 횟수가 아니라 화소당 min/max 연산 수**를 줄여야
// 합니다(vHGW O(1)). 다만 §18.6 계산으로는 그것도 `ir_signal` 기준 1.5~2배에 그쳐
// detect+apply 를 1,000ms 아래로 내리지 못합니다. 착수 전에 `D3D11_QUERY_TIMESTAMP` 로
// 커널 자체의 GPU 시간을 분리해 재십시오 — 지금 계측은 CPU 벽시계입니다.
// 바꿔도 값은 그대로여야 합니다.
//
