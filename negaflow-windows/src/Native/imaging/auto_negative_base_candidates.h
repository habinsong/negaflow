...
[[nodiscard]] std::optional<film_base_detail::BaseMeasurement> coherent_measurement(
    const film_base_detail::SampleGrid& grid,
    const std::vector<std::size_t>& selected);

// 가장자리를 따라 끊기지 않고 이어진 밝은 띠에서 베이스를 잽니다 - 스캔에 미노광
// 여백이 통째로 들어온, 가장 믿을 수 있는 경우입니다.
[[nodiscard]] std::optional<film_base_detail::BaseMeasurement> continuous_border_base(
    const film_base_detail::SampleGrid& grid,
...
// 2% 에 못 미치면 답하지 않습니다.
[[nodiscard]] std::optional<film_base_detail::BaseMeasurement> distributed_base(
    const film_base_detail::SampleGrid& grid,
    NegativeFilmType film_type,
    const std::vector<bool>* excluded);
