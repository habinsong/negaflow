...
internal static unsafe class ContractTestRunner
{
    ...
    private static void VerifyInfraredDetectorContract()
    {
        Check(sizeof(NativeInfraredDetectorParametersV1) ==
              NativeInfraredDefectDetector.ParametersV1Size, "infrared_parameters_size");
        Check(sizeof(NativeInfraredDetectionSummaryV1) ==
              NativeInfraredDefectDetector.SummaryV1Size, "infrared_summary_size");
        Check(sizeof(NativeInfraredClusterV1) ==
              NativeInfraredDefectDetector.ClusterV1Size, "infrared_cluster_size");
        Check(sizeof(NativeInfraredComponentV1) ==
        ...
        Check(result.Status == InfraredDetectionStatus.Ok, "infrared_detect_status");
        Check(result.Clusters.Count >= 1 && result.Components.Count >= 1,
              "infrared_owned_payload_copied");
        Check(result.Clusters[0].AttenuationR16.Length ==
    ...
    private static void VerifyFlatbedFrameGridContract()
    {
        Check(sizeof(NativeFlatbedFrameGridSummaryV1) == 24,
              "flatbed_summary_size");
        Check(sizeof(NativeFlatbedFrameDetectionV1) == 56,
        ...
        Check(result.Status == FlatbedFrameGridStatus.Ok && result.Detections.Count != 0,
              "flatbed_detects_owned_grid");
        Check(result.Detections.All(detection =>
