...
TiffProbeResult probe_tiff(
    ...
    try {
        ...
        if (!checked_multiply(
            ...
            !checked_add(
                result.info.first_ifd_offset, directory_count_bytes, entries_offset) ||
        ...
        std::array<std::uint8_t, 8> next_ifd_bytes{};
        if (!file.read(
                next_ifd_offset_position,
                next_ifd_bytes.data(),
                static_cast<std::size_t>(next_directory_bytes))) {
            result.status = TiffProbeStatus::truncated_ifd;
            return result;
        }
        const std::uint64_t next_ifd = result.info.variant == TiffVariant::classic
                                           ? read_u32(next_ifd_bytes.data(), result.info.byte_order)
                                           : read_u64(next_ifd_bytes.data(), result.info.byte_order);
        if (next_ifd != 0U) {
            result.status = TiffProbeStatus::multiple_directories_unsupported;
            return result;
        }
