...
| x64 native tests | 통과 | Debug/Release CTest 각각 28/28 통과 |
...
| scalar pointwise | 부분 구현 | exposure, RGB 3×4 matrix, 기본 톤·4-band curve, 고정 64표본 DR/R/G/B point curve, 8-band HSL Color Mixer x64 test·ARM64 build |
...
| M4 최소 tone | 첫 수직 경로 통과 | 노출→기본 톤→동적 band→파라메트릭 curve→point curve→Color Mixer native 경계, fixed Float32 fixture, TIFF16/PNG16 CLI report; 고급 조정 입력 UI/ABI와 실제 macOS golden은 미검증 |
| M4 단계 진단 | 첫 수직 경로 통과 | 기본 export stage wall/process-CPU, 진단 전용 scanner/develop/tone min/max·versioned 비암호 fingerprint, tone 24·point curve 24·Color Mixer 48-value conformance |
