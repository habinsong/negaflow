...
| x64 native tests | 통과 | Debug/Release CTest 각각 32/32 통과 |
...
| scalar pointwise·spatial | 부분 구현 | exposure, RGB 3×4 matrix, 기본 톤·4-band curve, 고정 64표본 DR/R/G/B point curve, 8-band HSL Color Mixer, 3구간 Color Grading, R/G/B Primary Calibration, standalone 11종 RGB33 Film Emulation 색상과 11행 acutance x64 test·ARM64 build |
| Film Emulation 색상·acutance | standalone component 통과 | 11종 profile, 33³ cube·5% 색상 step·431,244바이트 payload, unquantized acutance·11행 scratch; macOS golden 색상 최대 오차 `0.0018888685`, acutance 최대 오차 `0.00015372`; source routing·CLI·WinUI는 미연결 |
...
| M4 최소 tone | 첫 수직 경로 통과 | 노출→기본 톤→동적 band→파라메트릭 curve→point curve→Color Mixer→Color Grading→Primary Calibration native 경계, fixed Float32 fixture, TIFF16/PNG16 CLI report; Film Emulation macOS golden 확보, 고급 조정 입력 UI/ABI와 source route는 미검증 |
