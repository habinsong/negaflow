...
| 세로 슬라이스 (catalog→C ABI→WinUI) | 관통 확인 | 실행 중인 앱에서 frame 선택→노출 슬라이더→Export 로 현상된 PNG 생성. `Exported 631×403 in 142 ms`, 상태 표시줄 `ABI 0.3 · X64`. 시작 시 `library.sqlite` 생성·lock 획득 확인. import·미리보기·취소·진행률은 미구현 |
