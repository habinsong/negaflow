...
negaflow 是一个 macOS 和 Windows 应用。导入或扫描胶片图像之后，接着是反转、显影、GrainMend、输出和保存。 所有编辑都和原始文件分开存放。
...
按文件夹查看时使用**一个**网格，每个文件夹是一个分区，标题条是分区表头。请保持这个结构。如果给每个文件夹单独创建网格再纵向堆叠，布局就需要计算整个文件夹的高度，文件夹一进入画面便会创建全部卡片。共用一个网格时，可以按显示范围逐行创建卡片，让数百张照片的图库保持流畅滚动。
