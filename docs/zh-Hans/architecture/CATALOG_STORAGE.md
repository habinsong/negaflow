...
主存储是 `library.sqlite`。旧的 `library.json` 只用于迁移旧目录和生成诊断文件。 没有同时更新两个文件的做法，所以也没有 `dual-write`。
...
测量时间 2026-07-12，环境是 Mac14,3、arm64、8 核、24 GiB 内存、macOS 26.5、Swift Release 构建。 这些数字用于在相同环境下检查性能回退，不能预测其他 Mac 的速度。
...
迁完之后不会自动退回 JSON。 为了防止旧版本应用改动 JSON、造成两份存储不一致，会检查最低读取版本和迁移标记。
