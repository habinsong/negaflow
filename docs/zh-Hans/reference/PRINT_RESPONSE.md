...
曲线在整个区间里一直上升。`d ≥ 0` 时输出落在 `[baseToe, ceiling)` 内。比片基更亮的背光或齿孔
那种小于 0 的值不会被截成 0，而是继续保持有限的正值。
...
`normalRange` 不是胶片的物理最大密度，而是正常曝光场景用到的范围。它主要在测不到片基、或场景
对比度很低时起作用。
...
以前是分三段的函数加固定预设。v4 换成一条曲线加四个基准点，没有分段边界，每个值都能在代码和
测试里追到。
