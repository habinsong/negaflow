...
JPEG は、エンコーダーのサブサンプリングしきい値を超えないかぎり、色を輝度より低い解像度で
保存します。そこから下では色の解像度が水平・垂直とも半分になり、輝度のディテールはそのままに
彩度の高い境界がにじみます。そのため品質 95% 以上はクロマサブサンプリングなしでエンコードします。
それより低い設定は選んだ値をそのまま使います。低い品質を選ぶ意図はファイルを小さくすることだからです。

PNG と TIFF は可逆でサブサンプリングをしません。画質を決める値はチャンネルあたり 8bit か 16bit の
ビット深度だけです。ディザリングは量子化バンディングを隠す 8bit のときにのみ適用します。
