...
主な保存先は`library.sqlite`です。 以前の`library.json`は、古いカタログを移すときと診断用ファイルを作るときにだけ使います。 2つのファイルを同時に更新することはないので、`dual-write`はありません。
...
いまは`journal_mode=DELETE`、`synchronous=FULL`です。 WALだとデータベースと`-wal`ファイルをひとまとまりで扱う必要があります。 実行中のデータベースは直接コピーしません。 接続を閉じてから確認した主ファイルだけを復旧用コピーにします。
