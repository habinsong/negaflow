...
negaflowは、フィルム画像を取り込むかスキャンしたあと、反転、現像、GrainMend、出力、保存へとつな
がるmacOSアプリです。
すべての編集は原本とは別に保存します。
...
ファイルパスだけで原本を判断しません。
ファイルの観測値、バイト数、更新時刻、SHA-256、persistent bookmarkのように、いまの規格に必要な値
を残します。

ファイルが移動していたら、ユーザーがつなぎ直したときか、bookmarkの復旧が成功したときだけパスを変
えます。

## カタログ

主な保存先は`library.sqlite`です。
以前の`library.json`は、確認済みの古い資料を移すときと、持ち運べるバックアップを作るときに使いま
す。
2つの保存先を同時に更新することはありません。
...
フォルダ別表示では、フォルダごとに帯が付きます。三角、フォルダ、名前、枚数、現像プロセス、
ターゲット、適用。三角でその下のサムネイルを畳みます。畳んだフォルダは次回起動でも畳まれたまま
開き、サイドバーのファイル一覧の開閉とは別に記憶します。

フォルダ別表示は**グリッド1つ**で、フォルダがセクション、帯がそのヘッダです。この構造を保って
ください。フォルダごとにグリッドを分けて縦に積むと遅延が失われます。積む側がフォルダ1つ分の
高さを丸ごと知る必要があるため、フォルダが画面に掛かった瞬間にそのカードが全部作られます。
グリッドが1つなら遅延の単位は1行になり、数百枚のライブラリでもスクロールが滑らかに保たれます。
...
登録した物理ソースフォルダーごとにファイルシステム監視を1つ置きます。通知を短時間まとめた後、
変更されたフォルダーだけを読み直します。Finderで移動または名前変更されたソースはブックマーク
から同じカタログフォルダーIDへ再接続し、直下に追加された画像だけを取り込みます。ポーリングや
ライブラリ全体の再走査は行いません。
...
1枚と3つの歴史的レイアウトは、選択した写真ごとにページを作り、縦に並べて表示します。
コンタクトシート、ピクチャーパッケージ、カスタムパッケージは完成ページ数を表示し、その単位で
書き出します。39枚なら6列×7行のコンタクトシートは1ページ、4面付けパッケージは10ページ、
既定のカスタムパッケージは1ページ、個別レイアウトは39ファイルです。

パッケージプレビューは既存のサムネイルと現像画像を再利用し、不足時だけ小さな高速プレビューを
作ります。最終出力はメタデータから配置を計算し、各セルに必要な画素だけを現像して、固有ソースを
一度に2〜4件ずつ準備します。Core Imageグラフはページ書き込みまで維持し、ページごとのソース
ラスタ予算を512 MiBに制限します。

パッケージの各配置は、割り当てられたフレームを個別に監視します。ページの配置を終えたあと、
完成したページ全体にプリンター出力ICCを1回だけ適用するため、同じ写真の繰り返しでも複数写真の
組み合わせでも同じ出力規則になります。ライブラリと現像のプレビューには適用しません。
...
RFC 8493の BagIt構造とSHA-256の一覧を使い、すべてのファイルと関係を確認してから最終的な場所へ移
します。
...
アプリが一時的な場所を渡し、形式、サイズ、ID、実際に適用された設定を確認してから、アプリの保存先
へ移します。
