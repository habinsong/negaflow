...
エンコーダーが書き込みを終えたら、ImageIOで開き直してピクセルサイズを確認し、ファイル全体のハッ
シュを計算します。
サイドカーはそのあとに書きます。v3の検査に落ちた場合、完成した出力一式としては公開しません。
...
C2PAのhard bindingと処理履歴の考え方、PREMISの完全性の考え方は参考にしましたが、入るのは確認でき
るSHA-256の値だけです。
