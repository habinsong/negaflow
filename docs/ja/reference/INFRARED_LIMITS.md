...
`FilmType`はカラーと白黒、ネガとポジしか区別しません。
色素式の白黒と銀塩、通常のスライドと Kodachromeを分ける情報はありません。
...
色素式の白黒や通常のカラースライドでIRが絶対に使えない、という意味ではありません。
今ある情報ではフィルムの材質を確認できないので、推測しないだけです。
...
実機での確認は [実機点検リスト](../validation/REAL_QA_CHECKLIST.md)のIR項目に従います。
