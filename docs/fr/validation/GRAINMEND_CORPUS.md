...
`Config/defect-corpus-film-r-v2.json` fige la version DOI, la licence,
le nombre de paires et la taille totale.
...
Si le CDN de fichiers de Figshare bloque les requêtes automatiques,
téléchargez le ZIP depuis la page du jeu de données avec `Download all` et vérifiez-le tel quel.
L'extraction n'aboutit que si les noms de fichiers,
les tailles et les MD5 Figshare du ZIP correspondent tous au contrat figé.
...
Placez les fichiers endommagés et les restaurations, dont le nom se termine par `_restored`,
...
Sans cette option, il produit aussi `before`, `after`, `diff`,
`mask` et des recadrages à 100 % pour la vérification manuelle.
...
La décision finale demande de regarder `before`, `after`, `diff`,
`mask` et les recadrages à 100 % côte à côte.
...
Il ne prouve ni le décodage RAW, ni l'exactitude de l'inversion du film, ni l'alignement IR,
ni le comportement d'un scanner réel.

## Résultat du 2026-07-25

Les 44 paires ont tourné sur un build Release avec `--metrics-only --crops 0`.
L'ancienne référence de régression à sensibilité 3.0 a été comparée à 0.7,
le chemin automatique de la version publiée.
...
Le chemin automatique de la version publiée descend à 0.7,
et la détection des micro-taches est désactivée par défaut.
Quand les candidats dépassent 2 % d'une tuile, les composantes qui la touchent sont écartées.
Si une tuile dépasse 5 %, ou si le total des candidats après filtrage dépasse 0, 06 %,
la réparation automatique n'est pas appliquée à cette photo.
L'utilisateur peut alors resserrer la zone avec le mode Guidé.

Cette limite de sécurité ne concerne que le mode Auto.
Elle ne restreint ni la plage de détection ni la réparation en mode Guidé, Pinceau,
Tampon de duplication ou IR.
...
Face à l'ancienne référence, ce passage améliore 23 images de plus, en dégrade 27 de moins,
et relève le pire cas de 17, 614 dB.
Six images restent malgré tout en dessous de la restauration d'expert en PSNR.
FILM-R fournit des dommages réels et des restaurations manuelles,
et porte aussi l'ambiguïté du jugement de restauration.
Le matériel et l'article sont sur [le projet FILM-R](https:
//daniela997.github.io/FilmDamageSimulator/) et [l'article FILM-R](https:
