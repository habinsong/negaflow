...
이것을 정하는 것은 네거티브냐 포지티브냐가 아니라 **무엇이 화상을 이루는가**입니다.
컬러 필름은 현상 과정에서 은을 표백해 없애고 색소만 남기며, 색소는 적외선을 통과시킵니다. 흑백
필름은 화상 자체가 은이라 적외선을 막으므로, 보정을 걸면 사진 자체를 커다란 결함으로 읽어 지워
버립니다.
...
`FilmType`은 염료식 흑백과 은염을 가르지 못하고, Kodachrome과 일반 슬라이드도 구분하지 못합니다.
그래서 두 경우는 사용자의 판단에 맡깁니다.

- 염료식 흑백은 흑백으로 스캔하므로, 필름 자체는 IR을 쓸 수 있어도 자동 적용은 꺼진 채로 둡니다.
  필름 종류만 보고 짐작하지 않습니다.
- Kodachrome은 컬러 슬라이드이므로 IR이 제공됩니다. 다만 E-6와 적외선 감쇠가 달라 결함이 과소·
  과대 보정될 수 있습니다. 결과가 이상하면 레이어를 끄십시오.
