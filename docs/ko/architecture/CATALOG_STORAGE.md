...
빌드입니다.
다른 Mac의 속도를 보장하는 수치는 아닙니다. 같은 환경에서 회귀를 찾기 위한 기준값입니다.
...
같은 자료의 메모리 검색 준비는 32.86 ms, 전체 이름 정렬은 86.01 ms, 필터 후 이름 정렬은 158.37
ms였습니다.
필터 투영을 네 번 연속 실행한 p50은 512.80 ms였습니다.
...
실행 중인 데이터베이스를 임의로 복사하지 않고, 연결을 닫은 뒤 확인한 기본 파일만 복구 사본으로
만듭니다.
