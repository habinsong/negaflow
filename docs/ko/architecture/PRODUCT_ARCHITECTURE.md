...
폴더별 보기에서는 폴더마다 띠가 붙습니다. 삼각형, 폴더, 이름, 장수, 현상 프로세스, 타깃,
적용. 삼각형으로 그 아래 썸네일을 접습니다. 접어 둔 폴더는 다음 실행에서도 접힌 채로 열리며,
사이드바 파일 목록의 접힘과는 따로 기억합니다.

폴더별 보기는 **격자 하나**이고, 폴더가 구획이며 띠가 그 구획의 머리입니다. 이 구조를 유지해야
합니다. 폴더마다 격자를 따로 두고 그것을 세로로 쌓으면 게으름이 사라집니다. 쌓는 쪽이 폴더
하나의 높이를 통째로 알아야 하므로, 폴더가 화면에 걸리는 순간 그 폴더의 카드가 전부 만들어집니다.
격자가 하나면 게으름의 단위가 한 줄이 되고, 수백 장짜리 라이브러리에서 스크롤이 부드럽게
유지되는 이유가 그것입니다.
...
등록한 실제 원본 폴더마다 파일시스템 감시기 하나를 둡니다. 변경 알림은 짧게 모은 뒤 바뀐 폴더만
다시 읽습니다. Finder에서 원본을 옮기거나 이름을 바꾸면 북마크를 이용해 같은 카탈로그 폴더 ID로
다시 연결하고, 바로 아래에 새로 생긴 이미지만 가져옵니다. 폴링이나 라이브러리 전체 재탐색은 하지
않습니다.
...
한 장과 세 역사 인화 레이아웃은 선택한 사진마다 페이지를 하나씩 만들고 세로로 이어 보여 줍니다.
콘택트 시트, 사진 패키지, 사용자 패키지는 완성 페이지 수를 표시하고 그대로 내보냅니다. 39장이라면
6 × 7 콘택트 시트 한 페이지, 4장 사진 패키지 10페이지, 기본 사용자 패키지 한 페이지 또는
개별 인화 파일 39개가 됩니다.

패키지 미리보기는 이미 있는 썸네일과 현상 이미지를 재사용하고, 없는 항목만 작은 빠른 프리뷰로
만듭니다. 최종 출력은 메타데이터로 배치를 계산하고 각 칸에 필요한 픽셀만 현상하며, 고유 원본을
한 번에 2~4개씩 준비합니다. Core Image 그래프는 페이지 기록까지 이어 두고 페이지당 원본 래스터
예산은 512 MiB로 제한합니다.

사진 패키지의 각 칸은 자신에게 지정된 프레임을 따로 관찰합니다. 페이지 배치를 끝낸 뒤 완성된
페이지 전체에 프린터 출력 ICC를 한 번 적용하므로 같은 사진을 반복하거나 여러 사진을 섞어도 모든
칸이 같은 출력 규칙을 따릅니다. 라이브러리와 현상 미리보기에는 적용하지 않습니다.
