...
<p align="center">
  ...
  <a href="#ダウンロード"><img src="https://img.shields.io/badge/version-1.1.6-EF8B26" alt="バージョン 1.1.6"></a>
...
| インストーラー | 対象 |
|---|---|
| `negaflow-1.1.6-mac-universal.pkg` | macOS 14 以降、Apple Silicon と Intel |
| `negaflow-1.1.6-mac-arm64.pkg` | macOS 14 以降、Apple Silicon のみ |
| `negaflow-1.1.6-win-x64.exe` | Windows 11 24H2 以降、x64 |
...
- フィルムベースの測定とカラー・白黒のネガ・ポジの現像
- 露出、コントラスト、カーブ、HSL、カラーグレーディングの補正
...
- 7種類のプリントレイアウト、用紙プレビュー、写真用・ISO 用紙、C-print 機能

## Chroma Engine

**Chroma Engine** はフィルムの反転と現像を受け持ちます。

ネガの現像前に、未露光の領域でフィルムベースを測ります。自動測定がずれた場所はスポイトで取るか RGB 値を調整すれば大丈夫です。
...
自動とガイドは周辺の質感の向きと構造を確認してから欠陥を埋めます。手すりや目地をキズと間違えて消すと、写真を傷めてしまいます。
...
スキャナープロファイルを15個同梱しています。自分で撮ったフィルムから作成し、928個のデータを記録しました。
