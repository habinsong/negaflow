...
final class ChromabaseTests: XCTestCase {
    ...
    func testImageTransformDisplayNameUsesPlainRotationDegrees() {
        XCTAssertEqual(ImageTransform(rotation: .deg0).displayName, "0")
        XCTAssertEqual(ImageTransform(rotation: .deg90).displayName, "90")
        XCTAssertEqual(ImageTransform(rotation: .deg180).displayName, "180")
        XCTAssertEqual(ImageTransform(rotation: .deg270).displayName, "270")
        XCTAssertEqual(ImageTransform(rotation: .deg90, flipHorizontal: true).displayName, "90 H")
    }
    ...
    func testScannerProfileRegistryLoadsGeneratedProfiles() {
        XCTAssertEqual(
            ScannerProfileRegistry.computedFileSHA256(
                named: "noritsu__color-nega__kodak-ultramax-400"
            ),
            "sha256:fd28650f8b41aed695fe78b4738779c20c5a89fbd783a9861b6531a94ef36320"
        )
    ...
    func testScannerProfileValidationStatusAllowsOnlyPairedValidatedForAutomaticUse() {
        XCTAssertFalse(ScannerProfileValidationStatus.draft.allowsAutomaticUse)
        XCTAssertFalse(ScannerProfileValidationStatus.realOnly.allowsAutomaticUse)
        XCTAssertFalse(ScannerProfileValidationStatus.pairedSmoke.allowsAutomaticUse)
        XCTAssertTrue(ScannerProfileValidationStatus.pairedValidated.allowsAutomaticUse)
    }

    func testScannerProfileMatcherKeepsRealOnlyProfilesAvailableForManualSelection() {
        let profiles = ScannerProfileRegistry.loadAll()
        ...
        XCTAssertTrue(matches.contains {
            $0.id == "noritsu__color-nega__kodak-portra-400" &&
                $0.validationStatus == .realOnly
        })
    }
    ...
    func testScannerProfileMatcherRejectsUnvalidatedProfilesForAutomaticSelection() {
        let profiles = ScannerProfileRegistry.loadAll()
        let id = ScannerProfileMatcher.preferredProfileID(
            target: .noritsu,
            filmType: .colorNegative,
    ...
    func testScannerProfileMatcherPrefersPairedValidatedSameFilmStock() {
        let profiles = ScannerProfileRegistry.loadAll().map { profile in
            var profile = profile
            if profile.id == "noritsu__color-nega__kodak-portra-400" {
                profile.validationStatus = .pairedValidated
            }
            return profile
        }
        let id = ScannerProfileMatcher.preferredProfileID(
    ...
    func testScannerProfileMatcherMapsVision3DminIDsToPairedValidatedKodakProfileKeys() {
        let profiles = ScannerProfileRegistry.loadAll().map { profile in
            var profile = profile
            if profile.id == "sp-3000__color-nega__kodak-vision3-250d" {
                profile.validationStatus = .pairedValidated
            }
            return profile
        }
    ...
    func testScannerProfileMatcherDoesNotKeepUnvalidatedCurrentProfileAutomatically() {
        let profiles = ScannerProfileRegistry.loadAll()
        let id = ScannerProfileMatcher.preferredProfileID(
            target: .noritsu,
            filmType: .colorNegative,
    ...
    func testScannerProfileMatcherKeepsPairedValidatedCurrentProfileWhenNoFilmMatchExists() {
        let profiles = ScannerProfileRegistry.loadAll().map { profile in
            var profile = profile
            if profile.id == "noritsu__color-nega__kodak-portra-800" {
                profile.validationStatus = .pairedValidated
            }
            return profile
        }
    ...
    /// Neutral 은 "룩"이 아니라 무보정 기본값이다: 모든 톤/색/텍스처 파라미터가 DevelopParameters
    /// 기본값(0)과 동일하고, additive 적용 경로로도 사용자 값과 필드별 동일(무프리셋과 등가)해야 한다.
    func testNeutralPresetIsTrueNoOp() {
        let neutral = PresetRegistry.load(named: "neutral")!
        let base = neutral.baseParameters
        let def = DevelopParameters()
        XCTAssertEqual(base.exposure, def.exposure, accuracy: 1e-12)
        XCTAssertEqual(base.contrast, def.contrast, accuracy: 1e-12)
        XCTAssertEqual(base.density, def.density, accuracy: 1e-12)
        XCTAssertEqual(base.highlight, def.highlight, accuracy: 1e-12)
        XCTAssertEqual(base.shadow, def.shadow, accuracy: 1e-12)
        XCTAssertEqual(base.warmth, def.warmth, accuracy: 1e-12)
        XCTAssertEqual(base.tint, def.tint, accuracy: 1e-12)
        XCTAssertEqual(base.colorDepth, def.colorDepth, accuracy: 1e-12)
        XCTAssertEqual(base.saturation, def.saturation, accuracy: 1e-12)
        XCTAssertEqual(base.grain, def.grain, accuracy: 1e-12)
        XCTAssertEqual(base.sharpness, def.sharpness, accuracy: 1e-12)
        XCTAssertEqual(base.halation, def.halation, accuracy: 1e-12)

        var overrides = DevelopParameters()
        overrides.exposure = 0.3; overrides.contrast = -0.2; overrides.warmth = 0.15
        overrides.saturation = 0.1; overrides.shadow = 0.25; overrides.highlight = -0.3
        let merged = DevelopParameters(preset: neutral, overrides: overrides)
        XCTAssertEqual(merged.exposure, overrides.exposure, accuracy: 1e-12)
        XCTAssertEqual(merged.contrast, overrides.contrast, accuracy: 1e-12)
        XCTAssertEqual(merged.warmth, overrides.warmth, accuracy: 1e-12)
        XCTAssertEqual(merged.saturation, overrides.saturation, accuracy: 1e-12)
        XCTAssertEqual(merged.shadow, overrides.shadow, accuracy: 1e-12)
        XCTAssertEqual(merged.highlight, overrides.highlight, accuracy: 1e-12)
    }

    /// 각 룩이 표방하는 실기 성격(문서화된 필름/인화 특성)을 파라미터 수준에서 검증한다.
    /// - Warm Lab: Fuji Frontier 계열 웜/골든 → warmth>0
    /// - Clear Chrome: Provia/Astia 계열 클린 슬라이드 → 약간 쿨(warmth<0), 또렷(sharpness↑), 깊은 흑(shadow<0)
    /// - Deep Slide: Velvia 계열 → 고채도(colorDepth↑), 고대비, 깊은 흑
    /// - Soft Print: RA-4 소프트 인화 → 저대비, 들린 흑(shadow>0), 명부 보호(highlight<0)
    /// - Rich Neutral: 중립 색상(warmth=tint=0)이되 밀도/색심도는 채움
    func testLookPresetsExpressIntendedCharacter() {
        func p(_ id: String) -> DevelopParameters { PresetRegistry.load(named: id)!.baseParameters }
        let warmLab = p("warm-lab")
        let clearChrome = p("clear-chrome")
        let deepSlide = p("deep-slide")
        let softPrint = p("soft-print")
        let richNeutral = p("rich-neutral")

        XCTAssertGreaterThan(warmLab.warmth, 0)

        XCTAssertLessThan(clearChrome.warmth, 0)
        XCTAssertGreaterThan(clearChrome.sharpness, 0.1)
        XCTAssertLessThan(clearChrome.shadow, 0)

        XCTAssertGreaterThan(deepSlide.colorDepth, richNeutral.colorDepth)
        XCTAssertGreaterThan(deepSlide.saturation, 0.05)
        XCTAssertGreaterThan(deepSlide.contrast, 0.2)
        XCTAssertLessThan(deepSlide.shadow, 0)

        XCTAssertLessThan(softPrint.contrast, 0)
        XCTAssertGreaterThan(softPrint.shadow, 0)
        XCTAssertLessThan(softPrint.highlight, 0)

        XCTAssertEqual(richNeutral.warmth, 0, accuracy: 1e-12)
        XCTAssertEqual(richNeutral.tint, 0, accuracy: 1e-12)
    }

    /// highlightRollOff(>0=명부 보호)는 params.highlight 음수(Lightroom recovery)로 매핑돼야
    /// 이름과 동작이 일치한다(과거 부호 미반전 → 양수 rollOff 가 명부를 오히려 밝히던 스멜 회귀 방지).
    func testHighlightRollOffProtectsHighlights() {
        let softPrint = PresetRegistry.load(named: "soft-print")!
        XCTAssertGreaterThan(softPrint.tone.highlightRollOff, 0)
        XCTAssertLessThan(softPrint.baseParameters.highlight, 0)
    }

    /// end-to-end: Warm Lab 로 현상한 결과는 Neutral 보다 R/B 비가 높아야(따뜻) 한다.
    func testWarmLabRendersWarmerThanNeutralEndToEnd() {
        let fixture = makeSyntheticColorNegativeFixture()
        let engine = ChromabaseEngine()
        func meanRatio(_ id: String) -> Double {
            var overrides = DevelopParameters(); overrides.filmType = .colorNegative
            var params = DevelopParameters(preset: PresetRegistry.load(named: id)!, overrides: overrides)
            params.filmType = .colorNegative
            let out = engine.develop(image: fixture.image, base: fixture.base, params: params)
            let px = renderRGBA8(out, width: fixture.width, height: fixture.height)
            var rs = 0.0, bs = 0.0
            for i in stride(from: 0, to: px.count, by: 4) { rs += Double(px[i]); bs += Double(px[i + 2]) }
            return rs / max(bs, 1)
        }
        XCTAssertGreaterThan(meanRatio("warm-lab"), meanRatio("neutral"),
                             "Warm Lab 는 중립보다 따뜻(R/B↑)해야 한다.")
    }

    /// end-to-end: Deep Slide 로 현상한 결과는 Neutral 보다 평균 채도(max-min)가 높아야 한다.
    func testDeepSlideRendersMoreSaturatedThanNeutralEndToEnd() {
        let fixture = makeSyntheticColorNegativeFixture()
        let engine = ChromabaseEngine()
        func meanChroma(_ id: String) -> Double {
            var overrides = DevelopParameters(); overrides.filmType = .colorNegative
            var params = DevelopParameters(preset: PresetRegistry.load(named: id)!, overrides: overrides)
            params.filmType = .colorNegative
            let out = engine.develop(image: fixture.image, base: fixture.base, params: params)
        ...
        XCTAssertGreaterThan(meanChroma("deep-slide"), meanChroma("neutral"),
                             "Deep Slide 는 중립보다 채도가 높아야 한다.")
    }
    ...
    func testDevelopSettingsPasteScopeCopiesExtendedFieldsByGroup() {
        var source = DevelopParameters()
        source.lightSourceProfileID = "v850-led"
        source.autoLevels = true
        source.autoNeutralBalance = true
        source.bwToning = BWToning(mode: .sepia, strength: 0.7)
        source.filmEmulation = .velvia50
        source.filmEmulationIntensity = 0.42
        source.localDodgeBurn = [
            LocalDodgeBurnAdjustment(
                id: UUID(uuidString: "00000000-0000-0000-0000-000000000001")!,
                mode: .dodge,
                amount: 0.4,
                mask: .radial(
                    center: LocalDodgeBurnPoint(x: 0.25, y: 0.75),
                    radius: 0.2,
                    feather: 0.3
                )
            ),
        ...
        XCTAssertEqual(base.autoNeutralBalance, source.autoNeutralBalance)
        ...
        XCTAssertEqual(color.filmEmulationIntensity, source.filmEmulationIntensity)
        ...
        XCTAssertNil(detail.lightSourceProfileID)
    }
    ...
    func testRejectedSidecarExportsStandardNegativeOneXMPRating() {
        var sidecar = Sidecar(filmType: .colorNegative, parameters: DevelopParameters())
        sidecar.rating = 5
        sidecar.pickState = .rejected

        let xmp = sidecar.xmpPacket()

        XCTAssertTrue(xmp.contains("xmp:Rating=\"-1\""))
        XCTAssertTrue(xmp.contains("negaflow:Rating=\"5\""))
        XCTAssertTrue(xmp.contains("negaflow:PickState=\"rejected\""))
    }
    ...
    func testScannerProfilesProduceDistinctToneAndColor() {
        ...
        XCTAssertGreaterThan(
            ...
            "프로파일 간 톤 분리가 2.0→1.59로 줄었다. 2026-07-17: photometric 반전 교정으로 " +
            "미드톤이 올바른(어두운) 위치에 앉으면서 같은 LUT 차이의 절대 바이트 폭이 다시 " +
            "줄었다(실측 0.96). 이 가드의 목적은 프로파일이 동일 출력으로 붕괴(≈0)하지 않는지 " +
            "확인하는 것 — 0.7 이상이면 두 프로파일이 시각적으로 구분된다."
        )
    }
    ...
    func testSidecarEncodesScannerProfileAndFilmBaseDiagnostics() throws {
        ...
        let diagnostics = try XCTUnwrap(decoded.scannerProfileGradeDiagnostics)
        XCTAssertTrue(diagnostics.toneCorrection.hasPrefix("gamma="))
        XCTAssertTrue(diagnostics.neutralCorrection.hasPrefix("rgb="))
        XCTAssertTrue(diagnostics.chromaCorrection.hasPrefix("saturation="))
        XCTAssertNotNil(diagnostics.toneGamma)
        XCTAssertNotNil(diagnostics.saturationAmount)
    ...
    func testSidecarWritesMinimalXMPPacket() throws {
        ...
        sidecar.backendUsed = "external-plugin"
        ...
        XCTAssertTrue(xmp.contains("xmp:CreatorTool=\"negaflow \(NegaflowProductVersion.current)\""))
    ...
    func testSidecarWriteAtomicallyReplacesExistingJSON() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("negaflow-sidecar-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: url) }
        try Data("stale".utf8).write(to: url)
        let sidecar = Sidecar(filmType: .bwNegative, parameters: DevelopParameters())
        ...
        XCTAssertEqual(decoded.filmType, FilmType.bwNegative.rawValue)
    }
    ...
    func testPositiveMainIsFlatMasterAndBaseGradeKeepsShoulder() {
        // 2026-07-14: main 포지티브 = 플랫 마스터(네거티브와 동일 계약) — 슬라이드 베이스
        // 그레이드(딥블랙 리프트 0.010 / 하이라이트 숄더 0.965)를 main 에 굽지 않는다.
        // 과거의 "암부 뭉개짐" 버그 가드는 베이스 그레이드 단위 검증으로 유지한다
        // (베이스 그레이드는 실기 에뮬레이션/print/rescue 렌더 타겟에서만 쓰인다).
        ...
        let row = height / 2
        // 플랫: 암부 계조 그대로, 순백/순흑 유지(숄더·리프트 없음).
        let deepShadow = luma(out, x: Int(Double(width) * 0.06), y: row, width: width)
        XCTAssertGreaterThan(deepShadow, 6, "flat master must keep deep-shadow detail")
        ...
        XCTAssertGreaterThan(highShadow, lowShadow + 10, "shadow gradation must be preserved")
        let whiteOut = luma(out, x: width - 1, y: row, width: width)
        XCTAssertGreaterThanOrEqual(whiteOut, 253, "flat master must not bake a highlight shoulder into pure white")
        let blackOut = luma(out, x: 0, y: row, width: width)
        XCTAssertLessThanOrEqual(blackOut, 2, "flat master must not lift pure black")
        let mid = luma(out, x: width / 2, y: row, width: width)
        XCTAssertLessThanOrEqual(abs(mid - 128), 8, "flat master midtone must stay near identity. mid=\(mid)")

        // 베이스 그레이드 자체(렌더 타겟용 베이스): 암부 뭉개짐 금지 + 명부 숄더 유지.
        let graded = renderRGBA8(
            PositiveDevelop.applyBaseGrade(to: ramp, filmType: .colorPositive),
            width: width, height: height
        )
        let gradedDeep = luma(graded, x: Int(Double(width) * 0.06), y: row, width: width)
        XCTAssertGreaterThan(gradedDeep, 6, "base grade must keep deep-shadow detail instead of crushing to black")
        let gradedWhite = luma(graded, x: width - 1, y: row, width: width)
        XCTAssertLessThan(gradedWhite, 252, "base grade highlights must roll off with a shoulder")
    }
    ...
    func testColorNegativeInversionMapsDenseNegativeBrighterThanClearBase() {
        ...
        let expectedDenseLuma = [
            genericDensityCodeByte(transmission: 90.0 / 255.0, base: 224.0 / 255.0),
            genericDensityCodeByte(transmission: 72.0 / 255.0, base: 158.0 / 255.0),
            genericDensityCodeByte(transmission: 54.0 / 255.0, base: 107.0 / 255.0),
        ].reduce(0, +) / 3

        let expectedBaseLuma = genericDensityCodeByte(
            transmission: base.rgb.x,
            base: base.rgb.x
        )
        XCTAssertEqual(clearBaseLuma, expectedBaseLuma, accuracy: 1,
                       "필름 베이스는 paper-black 위의 고정 토우 값이어야 한다.")
        XCTAssertEqual(denseLuma, expectedDenseLuma, accuracy: 2,
                       "고밀도 패치는 shoulder-print-response-v4 기준값과 일치해야 한다.")
        XCTAssertGreaterThan(denseLuma, clearBaseLuma,
                             "더 높은 네거티브 밀도가 더 밝은 양화로 가는 순서가 뒤집히면 안 된다.")
    }
    ...
    func testNegativeInversionRetainsMidtoneDyeSeparation() {
        ...
        let expectedWarm = (
            genericDensityCodeByte(transmission: 82.0 / 255.0, base: base.rgb.x),
            genericDensityCodeByte(transmission: 54.0 / 255.0, base: base.rgb.y),
            genericDensityCodeByte(transmission: 38.0 / 255.0, base: base.rgb.z)
        )
        let expectedCool = (
            genericDensityCodeByte(transmission: 118.0 / 255.0, base: base.rgb.x),
            genericDensityCodeByte(transmission: 78.0 / 255.0, base: base.rgb.y),
            genericDensityCodeByte(transmission: 19.0 / 255.0, base: base.rgb.z)
        )

        // 임의의 "채도 N 이상"을 룩 품질로 간주하지 않는다. 두 패치의 모든 채널이 선언된
        // 상대밀도 reference와 일치하고, 원래의 warm/cool 방향만 보존되는지를 검증한다.
        XCTAssertEqual(Int(warm.0), expectedWarm.0, accuracy: 2)
        XCTAssertEqual(Int(warm.1), expectedWarm.1, accuracy: 2)
        XCTAssertEqual(Int(warm.2), expectedWarm.2, accuracy: 2)
        XCTAssertEqual(Int(cool.0), expectedCool.0, accuracy: 2)
        XCTAssertEqual(Int(cool.1), expectedCool.1, accuracy: 2)
        XCTAssertEqual(Int(cool.2), expectedCool.2, accuracy: 2)
        XCTAssertGreaterThan(warm.0, warm.2, "황갈색 패치의 warm 방향이 뒤집히면 안 된다.")
        XCTAssertGreaterThan(cool.2, cool.0, "청색 패치의 cool 방향이 뒤집히면 안 된다.")
    }

    /// 중립 회색 장면(밀도 공간에서 세 채널이 같은 밀도)은 반전 후 중립 회색이어야 한다.
    /// 이 모델에서는 Dmin 제거(밀도 정규화)가 WB를 잡고, 그 이후 인화 응답은
    ...
    /// 쓰므로 장면 독립적 → 보라와 염료 분리를 동시에 만족시킨다(Dmin/Dmax = 필름 고정값 모델).
    ...
    func testNegativeInversionFilmPresetResolvesBlueSkyHueShift() {
        ...
        // 중립 회색의 채널별 밀도는 같은 값이 아니라 채널 감마(∝ 프리셋 dmaxNorm 비율)에
        // 비례한다 — 그래서 dmaxNorm 채널별 정규화가 WB 를 만드는 것이다. 같은 밀도를 세
        // 채널에 넣는 픽스처는 프리셋 물성과 모순돼 반전이 캐스트를 "만든 것처럼" 보인다.
        let norm = fuji.dmaxNorm
        let grayDensity = (0.72 * norm.x / norm.y, 0.72, 0.72 * norm.z / norm.y)
        // float 픽스처 — 8bit 는 딥밀도(하늘 B 1.30D → byte 3~4)의 채널 독립 반올림이
        // photometric 지수(K)로 증폭돼, 프리셋의 하늘 오염 회피(이 테스트의 의도)가 아니라
        // 입력 양자화를 재게 된다.
        ...
        for y in 0..<height {
            for x in 0..<width {
                ...
                if x < width * 3 / 4 {
                    ...
                    d = grayDensity          // 중립 중간톤(채널 감마 비례 밀도)
                }
                floats[i]     = Float(baseRGB.x * pow(10, -d.0))
                floats[i + 1] = Float(baseRGB.y * pow(10, -d.1))
                floats[i + 2] = Float(baseRGB.z * pow(10, -d.2))
                floats[i + 3] = 1
            }
        }
    ...
    /// 업계 반전 도구들의 관례처럼 Dmin=필름 베이스(엣지 미노광) 고정 + 인화 곡선 RGB 공통.
    ...
    func testNegativeInversionMapsExposedLowlightToBlackWhenBaseIsInsideFrame() {
        ...
        let lowlight = luma(rendered, x: 20, y: 25, width: width)
        let denseFactor = 0.18 + Double(25) / Double(height - 1) * 0.30
        let dense = luma(rendered, x: 70, y: 25, width: width)
        XCTAssertEqual(lowlight, genericDensityCodeByte(transmission: base.x * 0.74, base: base.x),
                       accuracy: 2)
        XCTAssertEqual(dense, genericDensityCodeByte(transmission: base.x * denseFactor, base: base.x),
                       accuracy: 2)
        XCTAssertGreaterThan(dense, lowlight,
                             "프레임 안 베이스가 있어도 더 높은 밀도의 장면 순서가 뒤집히면 안 된다.")
    }
    ...
    func testNegativeInversionKeepsShadowToeInsteadOfClippingLowlightSteps() {
        ...
        XCTAssertEqual(clearBase, genericDensityCodeByte(transmission: base.x, base: base.x), accuracy: 1)
        XCTAssertEqual(lowlightA, genericDensityCodeByte(transmission: base.x * 0.78, base: base.x), accuracy: 2)
        XCTAssertEqual(lowlightB, genericDensityCodeByte(transmission: base.x * 0.68, base: base.x), accuracy: 2)
        XCTAssertEqual(highlight, genericDensityCodeByte(transmission: base.x * 0.20, base: base.x), accuracy: 2)
        XCTAssertLessThan(clearBase, lowlightA)
        XCTAssertLessThan(lowlightA, lowlightB)
        XCTAssertLessThan(lowlightB, highlight,
                          "서로 다른 밀도 단계가 클립되거나 순서가 뒤집히면 안 된다.")
    }
    ...
    /// 카메라 스캔(백라이트 위 필름 + 퍼포레이션): 베이스보다 밝은 비필름 노출광과 그
    /// 경계 혼합 셀을 베이스로 잡으면 안 된다 — 2026-07-14 비필름 마스크 + 팽창 검증.
    /// 512 폭 → 256 그리드 다운샘플로 퍼포레이션 경계 혼합 셀(중간 밝기·후보 판정 통과)을
    /// 실제로 만든다.
    func testFilmBaseEstimatorIgnoresBacklightAndPerforationsInCameraScan() {
        let width = 512
        let height = 384
        var bytes = [UInt8](repeating: 255, count: width * height * 4)
        for y in 0..<height {
            for x in 0..<width {
                let i = (y * width + x) * 4
                // 기본 = 백라이트(밝은 웜 화이트, luma ≈ 0.95).
                var r = 250, g = 244, b = 235
                let inFilm = y >= 96 && y < 320 && x >= 16 && x < 496
                if inFilm {
                    // 필름 베이스(오렌지 마스크).
                    r = 158; g = 107; b = 61
                    // 퍼포레이션: 필름 상단 대역의 구멍(백라이트가 그대로 보임).
                    let inPerfBand = y >= 112 && y < 136
                    if inPerfBand && (x % 48) < 24 {
                        r = 250; g = 244; b = 235
                    }
                    // 프레임(장면): 베이스보다 어둡다.
                    if y >= 160 && y < 288 && x >= 64 && x < 448 {
                        r = 64; g = 46; b = 31
                    }
                }
        ...
        guard let base = FilmBaseEstimator.estimate(from: image) else {
            return XCTFail("카메라 스캔에서 베이스 추정 실패")
        }
        // 진짜 베이스 (0.62, 0.42, 0.24) 근방이어야 하고, 백라이트(B 0.92)나
        // 퍼포레이션 혼합 셀(B 0.4~0.9) 쪽으로 새면 안 된다.
        XCTAssertEqual(base.rgb.x, 158.0 / 255.0, accuracy: 0.06, "R 이 베이스가 아니다: \(base.rgb)")
        XCTAssertEqual(base.rgb.y, 107.0 / 255.0, accuracy: 0.06, "G 가 베이스가 아니다: \(base.rgb)")
        XCTAssertEqual(base.rgb.z, 61.0 / 255.0, accuracy: 0.06, "B 가 베이스가 아니다: \(base.rgb)")
    }

    /// 흑백 네거티브 카메라 스캔: 중립 베이스(회색)와 중립 백라이트를 luma 로 구분해야 한다.
    func testFilmBaseEstimatorIgnoresBacklightForNeutralBWCameraScan() {
        let width = 512
        let height = 384
        var bytes = [UInt8](repeating: 255, count: width * height * 4)
        for y in 0..<height {
            for x in 0..<width {
                let i = (y * width + x) * 4
                var v = 242   // 백라이트(luma ≈ 0.95)
                let inFilm = y >= 96 && y < 320 && x >= 16 && x < 496
                if inFilm {
                    v = 140   // 중립 베이스(0.549)
                    let inPerfBand = y >= 112 && y < 136
                    if inPerfBand && (x % 48) < 24 { v = 242 }
                    if y >= 160 && y < 288 && x >= 64 && x < 448 { v = 60 }
                }
                bytes[i] = UInt8(v); bytes[i + 1] = UInt8(v); bytes[i + 2] = UInt8(v)
            }
        }
        let image = makeTestImage(
            bytes: bytes, width: width, height: height,
        ...
        guard let base = FilmBaseEstimator.estimate(from: image, neutralBase: true) else {
            return XCTFail("B&W 카메라 스캔에서 베이스 추정 실패")
        }
        XCTAssertEqual(base.rgb.x, 140.0 / 255.0, accuracy: 0.05, "베이스가 아니라 백라이트를 잡음: \(base.rgb)")
        let spread = max(abs(base.rgb.x - base.rgb.y), abs(base.rgb.y - base.rgb.z))
        XCTAssertLessThan(spread, 0.02, "B&W 베이스는 중립이어야 한다")
        XCTAssertLessThan(base.rgb.x, 0.75, "백라이트(0.95)로 새면 안 된다")
    }

    /// 실사용 실패 재현(2026-07-14): 백라이트 띠가 클리핑되지 않고(linear ≈ 0.84) 흰색-중립인
    /// 멀티 스트립 스캔 — 고정 임계값으로는 못 걸렀던 케이스. 응집 베이스 모드 기준 상대 컷이
    /// 띠·경계 혼합 셀을 걸러내야 한다.
    func testFilmBaseEstimatorHandlesDimUnclippedBacklightBands() {
        let width = 512
        let height = 384
        var bytes = [UInt8](repeating: 255, count: width * height * 4)
        for y in 0..<height {
            for x in 0..<width {
                let i = (y * width + x) * 4
                // 기본 = 어둑한 중립 백라이트(luma ≈ 0.84 — 절대 컷 0.88 아래).
                var r = 216, g = 214, b = 212
                let stripIndex = y / 128
                let yInStrip = y % 128
                let inFilm = yInStrip >= 12 && yInStrip < 108
                if stripIndex >= 3 || !inFilm {
                    // 스트립 사이 검정 갭(비필름).
                    if yInStrip >= 108 || stripIndex >= 3 { r = 4; g = 3; b = 3 }
                } else {
                    r = 158; g = 107; b = 61           // 베이스
                    if yInStrip < 24 && (x % 48) < 24 {  // 퍼포레이션(백라이트 노출)
                        r = 216; g = 214; b = 212
                    }
                    if yInStrip >= 36 && yInStrip < 96 && x >= 48 && x < 464 {
                        r = 60; g = 44; b = 30          // 프레임(장면)
                    }
                }
                bytes[i] = UInt8(r); bytes[i + 1] = UInt8(g); bytes[i + 2] = UInt8(b)
            }
        ...
        guard let base = FilmBaseEstimator.estimate(from: image) else {
            return XCTFail("어둑한 백라이트 스캔에서 베이스 추정 실패")
        }
        XCTAssertEqual(base.rgb.x, 158.0 / 255.0, accuracy: 0.06, "R: \(base.rgb)")
        XCTAssertEqual(base.rgb.y, 107.0 / 255.0, accuracy: 0.06, "G: \(base.rgb)")
        XCTAssertEqual(base.rgb.z, 61.0 / 255.0, accuracy: 0.06, "B: \(base.rgb)")
    }

    /// 어둑한 **웜** 백라이트(컬러 후보 판정까지 통과)는 강등 규칙(빈 갭 + 덜 오렌지)이
    /// 백라이트 모드를 버리고 진짜 베이스를 찾아야 한다.
    func testFilmBaseEstimatorDemotesWarmDimBacklight() {
        let width = 512
        let height = 384
        var bytes = [UInt8](repeating: 255, count: width * height * 4)
        for y in 0..<height {
            for x in 0..<width {
                let i = (y * width + x) * 4
                // 웜 백라이트: luma 0.66, R−B 0.12 — 컬러 후보 판정(≥0.06) 통과.
                var r = 184, g = 168, b = 153
                let inFilm = y >= 96 && y < 320 && x >= 16 && x < 496
                if inFilm {
                    r = 158; g = 107; b = 61
                    if y >= 112 && y < 136 && (x % 48) < 24 { r = 184; g = 168; b = 153 }
                    if y >= 160 && y < 288 && x >= 64 && x < 448 { r = 60; g = 44; b = 30 }
                }
                bytes[i] = UInt8(r); bytes[i + 1] = UInt8(g); bytes[i + 2] = UInt8(b)
            }
        }
        ...
        guard let base = FilmBaseEstimator.estimate(from: image) else {
            return XCTFail("웜 백라이트 스캔에서 베이스 추정 실패")
        }
        XCTAssertEqual(base.rgb.x, 158.0 / 255.0, accuracy: 0.06, "R: \(base.rgb)")
        XCTAssertEqual(base.rgb.z, 61.0 / 255.0, accuracy: 0.07, "B 가 백라이트로 새면 안 된다: \(base.rgb)")
    }

    /// 반전 통계 비필름 게이팅(2026-07-14): 멀티 스트립 스캔(백라이트 띠/검정 갭 포함)의
    /// 반전 결과가 필름만 잘라낸 크롭의 반전과 같아야 한다 — 과거엔 sampledDmin(p99.8)이
    /// 백라이트로 승격되고 blackInput(p90)이 광원에 끌려 결과가 물빠졌다(수동 base 로도 재현).
    func testNegativeInversionStatsIgnoreNonFilmRegions() {
        let stripW = 480, stripH = 180
        let base = SIMD3<Double>(0.62, 0.42, 0.24)
        // 필름 내용: 좌→우 중립 밀도 램프(0.15…1.45D) — 베이스 대비 감쇠.
        func filmPixel(_ x: Int) -> (Double, Double, Double) {
            let d = 0.15 + 1.3 * Double(x) / Double(stripW - 1)
            let a = pow(10.0, -d)
            return (base.x * a, base.y * a, base.z * a)
        }
        // (a) 필름만 있는 크롭.
        let cropBytesImage = makeFloatImage(width: stripW, height: stripH) { x, _ in filmPixel(x) }
        // (b) 같은 필름 + 위아래 백라이트 띠(0.9)와 검정 갭(0.01).
        let fullH = stripH + 120
        let fullImage = makeFloatImage(width: stripW, height: fullH) { x, y in
            if y < 40 { return (0.90, 0.89, 0.88) }             // 백라이트 띠
            if y < 60 { return (0.012, 0.010, 0.010) }          // 검정 갭
            if y >= 60 + stripH {
                return y < 60 + stripH + 20 ? (0.012, 0.010, 0.010) : (0.90, 0.89, 0.88)
            }
            return filmPixel(x)
        }
        let fb = FilmBase(rgb: base, source: .manual)
        let cropOut = renderRGBA8(
            NegativeInversion.apply(to: cropBytesImage, base: fb),
            width: stripW, height: stripH,
            colorSpace: CGColorSpace(name: CGColorSpace.sRGB)!
        )
        let fullOut = renderRGBA8(
            NegativeInversion.apply(to: fullImage, base: fb),
            width: stripW, height: fullH,
            colorSpace: CGColorSpace(name: CGColorSpace.sRGB)!
        )
        // 같은 필름 픽셀(스트립 중앙 행, 25%/50%/75% 지점)이 두 렌더에서 같아야 한다.
        let cropRow = stripH / 2
        let fullRow = 60 + stripH / 2
        for fraction in [0.25, 0.5, 0.75] {
            let x = Int(Double(stripW) * fraction)
            let ci = (cropRow * stripW + x) * 4
            let fi = (fullRow * stripW + x) * 4
            for c in 0..<3 {
                XCTAssertLessThanOrEqual(
                    abs(Int(fullOut[fi + c]) - Int(cropOut[ci + c])), 10,
                    "비필름 영역이 반전 통계를 오염시키면 안 된다 (x=\(x), ch=\(c), full=\(fullOut[fi + c]) crop=\(cropOut[ci + c]))")
            }
        }
    }

    private func makeFloatImage(width: Int, height: Int,
        ...
        for y in 0..<height {
            for x in 0..<width {
                let (r, g, b) = pixel(x, y)
                let o = (y * width + x) * 4
                pixels[o] = Float(r); pixels[o + 1] = Float(g); pixels[o + 2] = Float(b)
            }
        }
        return CIImage(
            bitmapData: Data(bytes: pixels, count: pixels.count * MemoryLayout<Float>.size),
    ...
    /// 멀티 스트립 카메라 스캔 공통 픽스처(1024×768, 스트립 3개 + 어둑한 중립 백라이트 띠
    /// + 퍼포레이션 + 검정 갭). gain 으로 광원 불균일(비네팅)을 모델링한다.
    /// 베이스 = (158,107,61)/255.
    private func makeMultiStripScan(gain: ((Int, Int) -> Double)? = nil) -> CIImage {
        let width = 1024, height = 768
        var bytes = [UInt8](repeating: 255, count: width * height * 4)
        for y in 0..<height {
            for x in 0..<width {
                let i = (y * width + x) * 4
                var r = 216.0, g = 214.0, b = 212.0        // 어둑한 백라이트(≈0.84)
                let stripIndex = y / 256
                let yInStrip = y % 256
                let inFilm = yInStrip >= 24 && yInStrip < 216 && stripIndex < 3
                if !inFilm {
                    if yInStrip >= 216 && yInStrip < 240 { r = 4; g = 3; b = 3 }   // 검정 갭
                } else {
                    r = 158; g = 107; b = 61               // 베이스
                    if yInStrip < 48 && (x % 96) < 48 {    // 퍼포레이션(백라이트 노출)
                        r = 216; g = 214; b = 212
                    }
                    if yInStrip >= 72 && yInStrip < 192 && x >= 96 && x < 928 {
                        r = 60; g = 44; b = 30             // 프레임(장면)
                    }
                }
                let k = gain?(x, y) ?? 1.0
                bytes[i] = UInt8(min(255, max(0, r * k)))
                bytes[i + 1] = UInt8(min(255, max(0, g * k)))
                bytes[i + 2] = UInt8(min(255, max(0, b * k)))
            }
        }
    ...
    /// 스포이드 스냅(2026-07-14 v3): 퍼포레이션 위, 검정 갭 옆, 베이스 위 — 클릭이 어디에
    /// 떨어져도 같은 베이스가 나와야 한다("찍을 때마다 랜덤" 증상 사멸).
    func testFilmBasePickerSnapsToBaseRegardlessOfClickAccuracy() {
        let image = makeMultiStripScan()
        // (x, y) 비트맵 좌표(y-down): 퍼포레이션 구멍 중앙 / 베이스 띠 / 검정 갭 경계 옆.
        let clicks: [(Double, Double, String)] = [
            (24.0 / 1024.0, (256.0 + 36.0) / 768.0, "퍼포레이션 구멍 위"),
            (512.0 / 1024.0, (256.0 + 58.0) / 768.0, "베이스 띠 위"),
            (512.0 / 1024.0, (216.0 + 6.0) / 768.0, "검정 갭 경계"),
        ]
        for (u, v, label) in clicks {
            guard let rgb = FilmBasePicker.sample(in: image, atUnit: CGPoint(x: u, y: v)) else {
                return XCTFail("\(label): 샘플 실패")
            }
            XCTAssertEqual(rgb.x, 158.0 / 255.0, accuracy: 0.05, "\(label) R: \(rgb)")
            XCTAssertEqual(rgb.y, 107.0 / 255.0, accuracy: 0.05, "\(label) G: \(rgb)")
            XCTAssertEqual(rgb.z, 61.0 / 255.0, accuracy: 0.05, "\(label) B: \(rgb)")
        }
    }

    /// 자동 모드 전체 파이프라인에서 미노광 베이스는 상대밀도 0에 놓이고, 장면 밀도는
    /// 베이스와 같은 값으로 붕괴하지 않아야 한다. 화면상 밝기 임계값은 룩 품질로 쓰지 않는다.
    func testAutoBaseMakesUnexposedBaseBlackInDevelopedOutput() {
        let width = 1024, height = 768
        let image = makeMultiStripScan()
        var params = DevelopParameters()
        params.filmType = .colorNegative
        let out = renderRGBA8(
            ChromabaseEngine().develop(image: image, base: nil, params: params),
            width: width, height: height
        )
        // 베이스 위치(스트립 1 베이스 띠, y-down 비트맵 → 출력 비트맵 동일 좌표계).
        let baseY = 256 + 58, baseX = 512
        let bi = (baseY * width + baseX) * 4
        let baseLuma = (Int(out[bi]) + Int(out[bi + 1]) + Int(out[bi + 2])) / 3
        XCTAssertLessThanOrEqual(baseLuma, 12,
            "미노광 베이스는 블랙이어야 한다. base out=(\(out[bi]),\(out[bi + 1]),\(out[bi + 2]))")
        // 프레임(장면) 위치는 베이스와 같은 검정으로 합쳐지지 않아야 한다.
        let sceneY = 256 + 128, sceneX = 512
        let si = (sceneY * width + sceneX) * 4
        let sceneLuma = (Int(out[si]) + Int(out[si + 1]) + Int(out[si + 2])) / 3
        XCTAssertGreaterThan(sceneLuma, baseLuma,
                             "장면 밀도가 미노광 베이스와 같은 값으로 붕괴함: base=\(baseLuma) scene=\(sceneLuma)")
    }

    /// 광원 불균일(비네팅 — 좌우 ±20% 조도 차) 아래에서도 성분 기반 검출은 베이스의 채널
    /// 비율을 유지해야 한다(고정 luma 창 방식은 베이스가 두 모드로 쪼개져 실패하던 조건).
    func testFilmBaseEstimatorSurvivesIlluminationFalloff() {
        let image = makeMultiStripScan(gain: { x, _ in 0.8 + 0.4 * Double(x) / 1023.0 })
        guard let base = FilmBaseEstimator.estimate(from: image) else {
            return XCTFail("비네팅 스캔에서 베이스 추정 실패")
        }
        let ratioRB = base.rgb.x / max(base.rgb.z, 1e-6)
        let expectedRatio = 158.0 / 61.0
        XCTAssertEqual(ratioRB, expectedRatio, accuracy: expectedRatio * 0.15,
            "베이스 R/B 비율 붕괴: \(base.rgb)")
        XCTAssertGreaterThan(base.rgb.x, 158.0 / 255.0 * 0.85, "베이스가 어두운 쪽으로 끌림: \(base.rgb)")
        XCTAssertLessThan(base.rgb.x, 158.0 / 255.0 * 1.30, "베이스가 광원으로 샘: \(base.rgb)")
    }

    /// 백라이트가 없는 필름 스캐너 크롭(밝은 것 = 베이스)은 마스크의 영향을 받지 않아야 한다.
    func testFilmBaseEstimatorMaskDoesNotAffectScannerCropWithoutBacklight() {
        let width = 96
        let height = 64
        var bytes = [UInt8](repeating: 0, count: width * height * 4)
        for y in 0..<height {
            for x in 0..<width {
                let i = (y * width + x) * 4
                let border = x < 6 || x >= width - 6 || y < 5 || y >= height - 5
                bytes[i] = border ? 200 : 70
                bytes[i + 1] = border ? 135 : 44
                bytes[i + 2] = border ? 85 : 28
                bytes[i + 3] = 255
            }
        }
        let image = makeTestImage(
        ...
        guard let base = FilmBaseEstimator.estimate(from: image, edgeFraction: 0.10) else {
            return XCTFail("필름 스캐너 크롭에서 베이스 추정 실패")
        }
        XCTAssertEqual(base.rgb.x, 200.0 / 255.0, accuracy: 0.05)
        XCTAssertEqual(base.rgb.y, 135.0 / 255.0, accuracy: 0.05)
        XCTAssertEqual(base.rgb.z, 85.0 / 255.0, accuracy: 0.05)
    }
    ...
    private func baseEstimatorFixture(baseRGB: SIMD3<Double>, width: Int = 64, height: Int = 48,
        ...
        // 픽스처 값은 linear 투과율 — 스캐너 raw 와 같은 의미론(linear 태그).
        return makeTestImage(bytes: bytes, width: width, height: height,
                             colorSpace: CGColorSpace(name: CGColorSpace.linearSRGB)!)
    }
    ...
    /// main = 플랫 마스터 계약(2026-07-05 재설계): 룩(muted-red 그레이드)·상시 chroma NR 을
    /// 베이스에 굽지 않는다. 채도는 염료 밀도 반전에서 그대로 나오고, 노이즈 제거는 opt-in
    /// (params.noiseReduction) 책임이다. 과거의 "기본 현상이 노이즈를 줄여야 한다" 가드는
    /// 이 계약과 상충해 제거했다 — 그 역할의 회귀 가드는 아래 opt-in NR 비교가 담당한다.
    func testColorNegativeMainTargetPreservesChromaAndDelegatesNoiseToOptInNR() {
        ...
        // MAIN 은 auto 경로의 scene-adaptive 반전(applySceneRanged)만 적용하고 별도 룩을 굽지
        // 않는다. reference 도 같은 반전을 써서 "MAIN = 반전 + 무룩" 계약을 검증한다.
        let expectedMain = renderRGBA8(
            NegativeInversion.applySceneRanged(to: fixture.image, base: fixture.base),
            width: fixture.width,
            height: fixture.height
        )
        let neutralPatch = patchPixels(pixels, width: fixture.width, x: 6..<30, y: 8..<32)
        let expectedRedPatch = patchPixels(expectedMain, width: fixture.width, x: 48..<72, y: 8..<22)
        let expectedGreenPatch = patchPixels(expectedMain, width: fixture.width, x: 48..<72, y: 28..<42)

        // 1. 탈색 금지: 한두 패치의 임의 채도 하한이 아니라, MAIN 전체 프레임이 선언된
        //    반전 reference와 동일한지 검증한다. reference 패치 자체의 색 방향도 확인해
        //    회색 픽스처끼리 비교하는 무의미한 테스트가 되지 않게 한다.
        XCTAssertGreaterThan(meanRedDominance(expectedRedPatch), 0)
        XCTAssertGreaterThan(meanGreenDominance(expectedGreenPatch), 0)
        XCTAssertEqual(
            meanChannelDifference(pixels, expectedMain),
            0,
            accuracy: 0.001,
            "MAIN 전체 프레임에 선언되지 않은 채도·톤 룩이 추가되면 안 된다."
        )

        // 2. 노이즈 제거는 opt-in: 기본 현상은 speckle 을 지우지 않고(정보 보존),
        //    사용자가 NR 을 켰을 때만 중립 패치의 chroma 노이즈가 줄어든다.
        var nrParams = params
        nrParams.noiseReduction = 1.0
        let nrPixels = renderRGBA8(
            ChromabaseEngine().develop(image: fixture.image, base: fixture.base, params: nrParams),
            width: fixture.width,
            height: fixture.height
        )
        let nrNeutralPatch = patchPixels(nrPixels, width: fixture.width, x: 6..<30, y: 8..<32)
        XCTAssertLessThan(
            meanChroma(nrNeutralPatch),
            meanChroma(neutralPatch) * 0.92,
            "opt-in NR 은 플랫 마스터의 중립 chroma 노이즈를 줄여야 한다. " +
            "default=\(meanChroma(neutralPatch)) nr=\(meanChroma(nrNeutralPatch))"
        )
    }
    ...
    func testLowRangeColorNegativeDevelopsToVisiblePositive() {
        ...
        let rendered = renderLinearRGBA8(output, width: width, height: height)
        // 저신호(얇은) 프레임에서 auto 파이프라인은 scene-adaptive 반전(applySceneRanged)을 쓴다.
        // MAIN 은 그 위에 별도 룩이 없으므로 전 프레임이 반전 결과와 일치해야 한다.
        let reference = renderLinearRGBA8(
            NegativeInversion.applySceneRanged(to: image, base: FilmBase(rgb: base, source: .border)),
            width: width,
            height: height
        )
        XCTAssertEqual(
            meanChannelDifference(rendered, reference),
            0,
            accuracy: 0.001,
            "저신호 프레임 전체가 auto 반전(applySceneRanged) 결과와 일치해야 한다(MAIN 무룩)."
        )

        let baseLuma = luma(rendered, x: 1, y: 1, width: width)
        let denseLuma = luma(rendered, x: 4, y: height / 2, width: width)
        let thinLuma = luma(rendered, x: width - 5, y: height / 2, width: width)
        XCTAssertEqual(
            baseLuma,
            genericDensityCodeByte(transmission: base.x, base: base.x),
            accuracy: 2
        )
        XCTAssertGreaterThan(denseLuma, thinLuma,
                             "저신호 입력에서도 네거티브 밀도 순서가 뒤집히면 안 된다.")
    }
    ...
    func testSoftwareDefectFillsAchromaticDustWithMedian() {
        let width = 32
        let height = 32
        let linear = CGColorSpace(name: CGColorSpace.linearSRGB)!
        var pixels = [Float](repeating: 1, count: width * height * 4)
        ...
        let output = SoftwareDefectRemoval.apply(to: input, threshold: 0.06, strength: 1.0)
        ...
        XCTAssertGreaterThan(center, 0.35, "결함 제거가 먼지를 median으로 채워야 함")
    ...
    func testImageLoaderHonorsScannerTIFFEmbeddedProfile() {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
        ...
        let loaded = try! XCTUnwrap(ImageLoader.loadScannerTIFF(tmp))
        XCTAssertNotEqual(
            loaded.colorSpace?.name,
            CGColorSpace(name: CGColorSpace.linearSRGB)?.name,
            "임베디드 sRGB 계열 프로파일을 linear raw로 덮어쓰면 안 된다."
        )
    }
    ...
    /// `NegativeInversion.shoulder-print-response-v4`의 독립 CPU reference를 sRGB byte로
    /// 변환한다. 계수는 공개 광도 계약 앵커(토우 0.001, 미드 0.18@0.60D/1.55, 화이트 0.70,
    /// 천장 0.90)에서 닫힌형으로 유도한다 — 구현을 참조하지 않는 독립 수식.
    private func genericDensityCodeByte(transmission: Double, base: Double) -> Int {
        let normalRange = 0.62 * 2.5
        let yCeil = log10(0.90)
        let amplitude = yCeil - log10(0.001)
        let rMid = log(amplitude / (yCeil - log10(0.18)))
        let rWhite = log(amplitude / (yCeil - log10(0.70)))
        let shape = log(rWhite / rMid) / log(normalRange / 0.60)
        let rate = pow(rWhite, 1.0 / shape)
        let density = log10(base / max(transmission, 1e-5))
        let normalized = max(density / normalRange, 0.0)
        let linear = pow(10.0, yCeil - amplitude * exp(-pow(rate * normalized, shape)))
        let code = linear <= 0.003_130_8
            ? 12.92 * linear
            : 1.055 * pow(linear, 1.0 / 2.4) - 0.055
        return Int((min(max(code, 0), 1) * 255).rounded())
    }
    ...
    private func makeNoisyMainTargetFixture() -> (image: CIImage, base: FilmBase, width: Int, height: Int) {
        ...
        let height = 50
        let base = SIMD3<Double>(0.86, 0.54, 0.34)
        // 물리 밀도 모델(C-41 특성곡선: D = D_mid + γ_c·log10(refl/0.18)) — 과거 토이 모델
        // (base×(1−p)^(1/0.72))은 채널 밀도 범위 스프레드가 ±26% 로 실제 필름 물성(±12%)을
        // 벗어나, dmaxNorm 스프레드 캡과 충돌해 채도 측정을 왜곡했다(2026-07-18).
        let filmGamma = SIMD3<Double>(0.65, 0.62, 0.60)
        let midDensity = 0.75
        func negativeT(_ positive: SIMD3<Double>) -> SIMD3<Double> {
            var t = SIMD3<Double>()
            for c in 0..<3 {
                let logE = log10(max(positive[c], 1e-6) / 0.18)
        ...
        for y in 0..<height {
            for x in 0..<width {
                ...
                let negative = negativeT(positive)
                let offset = (y * width + x) * 4
