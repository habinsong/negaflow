...
final class HighResolutionInteractionPerformanceTests: XCTestCase {
    func test7200DPIEditSequenceStaysResponsive() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["NEGAFLOW_INTERACTION_PERF"] == "1",
            "Set NEGAFLOW_INTERACTION_PERF=1 and use Release to run interaction timing gates."
        )
    ...
    func testLargeImportedRAWLikeImageEditAndDefectPathsStayResponsive() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["NEGAFLOW_INTERACTION_PERF"] == "1",
            "Set NEGAFLOW_INTERACTION_PERF=1 and use Release to run interaction timing gates."
        )
        ...
        let elapsed = timed {
            let field = SoftwareDefectRemoval.detectComponents(in: roiImage, roi: roi, parameters: params)
            ...
            _ = SoftwareDefectRemoval.repair(image: roiImage, roi: roi, mask: maskImage)
        }
        #if DEBUG
        let defectLimit: TimeInterval = 12.0
        #else
        let defectLimit: TimeInterval = 1.25
        #endif
        XCTAssertLessThan(elapsed, defectLimit, "brush/region defect-removal representative ROI work should remain ROI-bounded")
    }
