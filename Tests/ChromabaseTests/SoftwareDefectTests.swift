...
final class SoftwareDefectTests: XCTestCase {
    func testDetectMaskFindsThinHorizontalScratchOnMidtone() {
        ...
        let mask = SoftwareDefectRemoval.detectMask(
            in: image,
    ...
    func testDetectMaskBrushConstrainsToPaintedRegion() {
        ...
        let params = SoftwareDefectParameters(strength: 1, dustSensitivity: 0.7, scratchSensitivity: 0.9, protectDetail: 0.7)
        let mask = SoftwareDefectRemoval.detectMask(in: image, parameters: params, brush: brush)
        let rendered = renderLinearRGBA(mask, width: width, height: height)
    ...
    func testApplyWithBrushRepairExtentLeavesOutsidePixelsUnchanged() {
        ...
        let params = SoftwareDefectParameters(strength: 1, dustSensitivity: 0.75, scratchSensitivity: 0.95, protectDetail: 0.7)

        let repaired = SoftwareDefectRemoval.apply(
            to: image,
            parameters: params,
            brush: brush,
    ...
    func testOverlayMaskPaintsDetectedPixelsRed() {
        ...
        let overlay = SoftwareDefectRemoval.overlayMask(on: image, mask: mask, opacity: 0.8)
