...
// raw 스캔 도메인(linearSRGB)에서 브러시 결함 제거 가 결함을 제거하는지 검증한다.
// 사용자는 현상된 positive 에서 칠하지만 실제 적용은 raw 에 일어나므로(raw 단계 결함 제거),
// 선형 도메인에서, 그리고 네거티브에서 결함이 밝게/어둡게 나타나는 양극성 모두에서
// 검출·복원이 동작해야 한다.
final class RawDomainDefectTests: XCTestCase {
    ...
    /// 결함을 칠해 결함 제거 적용 → 결함 위치의 복원 후 밝기와 배경을 돌려준다.
