...
// 염료 분리를 동시에 만족시킨다(Dmin/Dmax 를 필름 고정값으로 두는 업계 공통 모델).
//
...
public struct FilmStockDmin: Codable, Sendable, Equatable, Identifiable {
    /// 밀도 값의 출처. 제조사 데이터시트에도 D-min 수치 테이블은 없고 특성곡선 차트에서
    /// 판독해야 한다(Portra 400 E-4050 실확인) — 즉 최선의 경우에도 "차트 판독 근사"다.
    /// 이를 숨기지 않고 명시한다. 정밀한 색은 스캔 실측 base가 담당한다(실측 우선 원칙).
    public enum Provenance: String, Codable, Sendable {
        /// 제조사 데이터시트 특성곡선에서 판독한 근사값.
        case datasheetCurve
        /// 공개 데이터 없음 — 유사 필름/공정에서 유추한 추정값.
        case estimated

        public var displayName: String {
            switch self {
            case .datasheetCurve: return "데이터시트 곡선 판독"
            case .estimated: return "추정값"
            }
        }
    }
    ...
    /// 밀도 값의 출처(코딩 제외 — 레지스트리 코드에서만 지정).
    public var provenance: Provenance = .estimated

    enum CodingKeys: String, CodingKey {
        case id, displayName, manufacturer, iso, process, dminDensity, dmaxDensity
    }
...
public enum FilmStockDminRegistry {
    ...
    //
    // 데이터 출처의 진실: 제조사 데이터시트에 D-min "수치 테이블"은 존재하지 않는다
    // (Portra 400 E-4050 실확인 — 특성곡선/스펙트럼 차트만 있음). 따라서 여기 값은
    //   • provenance == .datasheetCurve: 공개 특성곡선 차트에서 판독한 근사값(±0.05~0.1D)
    //   • provenance == .estimated:     공개 데이터 없음 — 유사 필름/공정 유추
    // 이며, Status M 농도 기준이라 스캐너 광원/센서에 따라 투과율은 달라진다.
    // → 색 정확도의 앵커는 항상 "스캔 실측 base"(스포이드/가장자리)이고, 프리셋의 역할은
    //   장면 독립적인 dmaxNorm(밀도 범위 = 필름 물성) 제공이다. Dmin 투과율은 실측이 없을
    //   때의 폴백일 뿐이다(LightSourceProfile로 광원 트림 가능).
    public static let all: [FilmStockDmin] = [
        ...
        .init(id: "kodak-portra-160", displayName: "Kodak Portra 160", manufacturer: "Kodak", iso: 160, process: "C-41",
              dminDensity: SIMD3(0.22, 0.62, 0.82), dmaxDensity: SIMD3(2.20, 2.80, 3.00), provenance: .datasheetCurve),
        .init(id: "kodak-portra-400", displayName: "Kodak Portra 400", manufacturer: "Kodak", iso: 400, process: "C-41",
              dminDensity: SIMD3(0.21, 0.62, 0.82), dmaxDensity: SIMD3(2.25, 2.85, 3.05), provenance: .datasheetCurve),
        .init(id: "kodak-portra-800", displayName: "Kodak Portra 800", manufacturer: "Kodak", iso: 800, process: "C-41",
              dminDensity: SIMD3(0.27, 0.70, 0.95), dmaxDensity: SIMD3(2.20, 2.75, 2.95), provenance: .datasheetCurve),
        .init(id: "kodak-ektar-100", displayName: "Kodak Ektar 100", manufacturer: "Kodak", iso: 100, process: "C-41",
              dminDensity: SIMD3(0.25, 0.62, 0.82), dmaxDensity: SIMD3(2.25, 2.85, 3.05), provenance: .datasheetCurve),
        .init(id: "kodak-gold-200", displayName: "Kodak Gold 200", manufacturer: "Kodak", iso: 200, process: "C-41",
              dminDensity: SIMD3(0.24, 0.65, 0.88), dmaxDensity: SIMD3(2.05, 2.60, 2.80), provenance: .datasheetCurve),
        .init(id: "kodak-ultramax-400", displayName: "Kodak UltraMax 400", manufacturer: "Kodak", iso: 400, process: "C-41",
              dminDensity: SIMD3(0.25, 0.65, 0.90), dmaxDensity: SIMD3(2.05, 2.60, 2.80), provenance: .datasheetCurve),
        .init(id: "kodak-pro-image-100", displayName: "Kodak Pro Image 100", manufacturer: "Kodak", iso: 100, process: "C-41",
              dminDensity: SIMD3(0.25, 0.65, 0.85), dmaxDensity: SIMD3(2.05, 2.60, 2.85), provenance: .datasheetCurve),
        ...
        .init(id: "fuji-c200", displayName: "Fujicolor C200", manufacturer: "Fujifilm", iso: 200, process: "C-41",
              dminDensity: SIMD3(0.20, 0.58, 0.88), dmaxDensity: SIMD3(1.85, 2.35, 2.65), provenance: .datasheetCurve),
        ...
        .init(id: "fuji-superia-400", displayName: "Fujifilm Superia X-TRA 400", manufacturer: "Fujifilm", iso: 400, process: "C-41",
              dminDensity: SIMD3(0.20, 0.58, 0.88), dmaxDensity: SIMD3(2.00, 2.60, 2.80), provenance: .datasheetCurve),
        .init(id: "fuji-100", displayName: "Fujicolor 100", manufacturer: "Fujifilm", iso: 100, process: "C-41",
              dminDensity: SIMD3(0.20, 0.58, 0.85), dmaxDensity: SIMD3(1.85, 2.35, 2.65), provenance: .datasheetCurve),
        // --- Kodak Vision3 (ECN-2, 영화용) ---
        .init(id: "vision3-50d", displayName: "Kodak Vision3 50D (5203)", manufacturer: "Kodak", iso: 50, process: "ECN-2",
              dminDensity: SIMD3(0.16, 0.60, 0.85), dmaxDensity: SIMD3(1.90, 2.70, 2.90), provenance: .datasheetCurve),
        .init(id: "vision3-200t", displayName: "Kodak Vision3 200T (5213)", manufacturer: "Kodak", iso: 200, process: "ECN-2",
              dminDensity: SIMD3(0.20, 0.62, 0.87), dmaxDensity: SIMD3(2.05, 2.75, 2.95), provenance: .datasheetCurve),
        .init(id: "vision3-250d", displayName: "Kodak Vision3 250D (5207)", manufacturer: "Kodak", iso: 250, process: "ECN-2",
              dminDensity: SIMD3(0.17, 0.62, 0.87), dmaxDensity: SIMD3(2.00, 2.70, 2.95), provenance: .datasheetCurve),
        ...
        .init(id: "harman-phoenix-200", displayName: "Harman Phoenix 200", manufacturer: "Harman", iso: 200, process: "C-41",
              dminDensity: SIMD3(0.22, 0.32, 0.40), dmaxDensity: SIMD3(1.70, 1.80, 2.15), provenance: .datasheetCurve),
        .init(id: "harman-phoenix-ii", displayName: "Harman Phoenix II", manufacturer: "Harman", iso: 200, process: "C-41",
              dminDensity: SIMD3(0.22, 0.32, 0.47), dmaxDensity: SIMD3(1.70, 1.80, 2.15), provenance: .datasheetCurve),
