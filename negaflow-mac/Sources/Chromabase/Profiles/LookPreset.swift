...
public struct LookPreset: Codable, Sendable, Identifiable, Equatable, Hashable {
    ...
    public var baseParameters: DevelopParameters {
        ...
        // highlightRollOff > 0 은 명부를 보호(shoulder 로 압축)한다는 뜻이다. params.highlight 는
        // 일반 규약(양수=명부를 더 밝게, 음수=recovery)이므로 부호를 반전해 매핑한다.
        // (반전 없이 넣던 기존 매핑은 rollOff 양수가 오히려 명부를 밝혀 이름과 정반대로 동작했다.)
