...
// 컴포넌트(연결요소) 라벨맵 — 가이드 영역 결함 제거 전용.
//
// 브러시 결함 제거는 검출 마스크(RGBA8)를 바로 복원에 쓴다(DefectComponentMask.build). 영역 결함 제거는
// 사용자가 개별 결함을 클릭으로 제외해야 하므로, 마스크를 "라벨링된 컴포넌트 목록"으로 들고
// 있어야 한다. 게이트(면적/aspect/길이) 통과 판정은 DefectComponentMask 와 동일 로직을 공유한다
// (DefectComponentMask.buildLabeled). 여기서는 결과 타입과 좌표 조회만 정의한다.

public struct DefectComponent: Sendable {
    ...
    public let maxX: Int
    public let maxY: Int
    /// 물리 결함 분류(v2). 검출 직후 DefectClassifier 가 채운다 — 기본값은 kind 별 대표 분류.
    public let classification: DefectClass
    /// 검출 증거 세기(0~1). strong 코어 비율 + SNR 요약 — UI 표시·정렬용이며 게이트가 아니다.
    public let confidence: Double

    init(id: Int32, kind: Kind, pixels: [Int], minX: Int, minY: Int, maxX: Int, maxY: Int,
         classification: DefectClass? = nil, confidence: Double = 0.5) {
        self.id = id
        self.kind = kind
        self.pixels = pixels
        self.minX = minX
        ...
        self.maxX = maxX
        self.maxY = maxY
        self.classification = classification ?? (kind == .dust ? .dust : .scratchHorizontal)
...
public struct DefectLabelField: Sendable {
