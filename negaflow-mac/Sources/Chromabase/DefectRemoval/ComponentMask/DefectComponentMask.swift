...
enum DefectComponentMask {
    ...
    /// 먼지 strong 코어 밀도 하한. strong 픽셀이 "하나라도" 있으면 채택하던 계약은 컴포넌트가
    /// 커질수록 무의미해진다 — 수만 px 짜리 덩어리도 코어 몇 개면 통과했다. 실제 이물은 코어가
    /// 몸통을 채우지만, weak 연결로 이어 붙은 그레인·텍스처 카펫은 코어가 드문드문하다. 비율로
    /// 보면 작은 결함은 자연히 통과하고(3px 에 코어 1개 = 0.33) 카펫만 걸린다.
    ///
    /// 필요한 이유(2026-07-26 실측): 가이드 경로는 먼지 면적 상한을 물리 크기의 49배까지 연다
    /// (5000px 스캔에서 238×238px). 흑백 필름처럼 그레인이 굵은 장면에서 텍스처가 통째로 한
    /// 컴포넌트로 뭉치면 그 상한을 통과해 ROI 가 뭉개졌다 — "면적 게이트가 카펫을 막는다"는
    /// 설계 전제가 확장된 상한 아래에서 무너져 있었다.
    static let dustMinStrongFraction = 0.08
