...
struct ToneCurveEditor: View {
    ...
    enum Channel: String, CaseIterable, Identifiable {
        ...
        func label(language: AppLanguage) -> String {
            switch self {
            case .rgb: return AppLocalization.text(AppLocalizedPhrase.dynamicRangeChannel, language: language)
            case .red: return AppLocalization.text(AppLocalizedPhrase.red, language: language)
            case .green: return AppLocalization.text(AppLocalizedPhrase.green, language: language)
            case .blue: return AppLocalization.text(AppLocalizedPhrase.blue, language: language)
            }
        }
        var tint: Color {
            switch self {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            CurveCanvas(
                ...
                .frame(height: 188)
            ToneCurvePointFields(
                points: pointsBinding,
                selectedIndex: $selectedPointIndex,
                onChange: onChange
            )
            HStack {
                Text(model.text(AppLocalizedPhrase.toneCurveEditorHelp))
                    .font(.caption2)
        ...
        .onChange(of: channel) { _, _ in
            selectedPointIndex = currentPoints.isEmpty ? nil : 0
        }
    }
        ...
        HStack(spacing: 2) {
            ForEach(Channel.allCases) { ch in
                Button {
                    ...
                    Text(ch.label(language: model.appLanguage))
...
private struct CurveCanvas: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                ForEach(Array(effectivePoints.enumerated()), id: \.offset) { idx, p in
                    Circle()
                        .fill(tint)
                        .overlay(Circle().stroke(.white.opacity(0.85), lineWidth: selectedIndex == idx ? 2 : 1))
        ...
        .accessibilityLabel(model.text(AppLocalizedPhrase.toneCurve))
        .accessibilityValue(accessibilityValue)
        .accessibilityAdjustableAction { direction in
            switch direction {
            case .increment: nudge(.up, step: 0.01)
            case .decrement: nudge(.down, step: 0.01)
            @unknown default: break
            }
        }
        .accessibilityAction(named: Text(model.accessibilityText(.previousPoint))) {
            selectAdjacent(-1)
        }
        .accessibilityAction(named: Text(model.accessibilityText(.nextPoint))) {
            selectAdjacent(1)
        }
        .accessibilityAction(named: Text(model.accessibilityText(.addPoint))) {
            ToneCurvePointEditing.addPoint(points: &points, selectedIndex: &selectedIndex)
            onChange()
        }
        .accessibilityAction(named: Text(model.accessibilityText(.deletePoint))) {
            ToneCurvePointEditing.deleteSelected(points: &points, selectedIndex: &selectedIndex)
            onChange()
        }
        .focusable()
        .onKeyPress(.leftArrow, phases: [.down, .repeat]) { handleKey(.left, press: $0) }
        .onKeyPress(.rightArrow, phases: [.down, .repeat]) { handleKey(.right, press: $0) }
        .onKeyPress(.upArrow, phases: [.down, .repeat]) { handleKey(.up, press: $0) }
        .onKeyPress(.downArrow, phases: [.down, .repeat]) { handleKey(.down, press: $0) }
    }
    ...
    private func removePoint(near location: CGPoint, in rect: CGRect) {
        ...
        points.remove(at: i)
        selectedIndex = min(i, points.count - 1)
        onChange()
    }

    private var accessibilityValue: String {
        guard let index = selectedIndex, points.indices.contains(index) else {
            return model.accessibilityText(.addPoint)
        }
        return model.accessibilityText(
            .curvePointValueFormat,
            index + 1,
            points.count,
            Int((points[index].x * 100).rounded()),
            Int((points[index].y * 100).rounded())
        )
    }

    private func handleKey(_ direction: ToneCurvePointNudge, press: KeyPress) -> KeyPress.Result {
        nudge(direction, step: press.modifiers.contains(.shift) ? 0.05 : 0.01)
        return .handled
    }

    private func nudge(_ direction: ToneCurvePointNudge, step: Double) {
    ...
    private func selectAdjacent(_ offset: Int) {
        ToneCurvePointEditing.selectAdjacent(
            points: points,
            selectedIndex: &selectedIndex,
            offset: offset
        )
