...
final class ChromaRecombineAlphaTests: XCTestCase {
    ...
    func testFullRangeRampPreservesShadowAndHighlightGradation() {
        // 높이는 반전 통계 샘플링 최소 크기(> 8)를 넘겨야 한다 — 8 이면 sampleStats 가
        // nil 로 떨어져 fallback 통계(dmaxNorm 1.2)가 램프 실측(≈1.6)과 어긋나고, 램프
        // 상단이 통째로 오버레인지 테일로 밀려 이 가드의 목적과 무관한 실패가 난다.
        let width = 256, height = 16
        let base = SIMD3<Double>(0.86, 0.54, 0.34)
        var bytes = [UInt8](repeating: 0, count: width * height * 4)
        for y in 0..<height { for x in 0..<width {
            // 밀도 램프 0..1.6 ≈ 8.9 스탑 장면(γ0.6) — 고대비 실장면 상한. 과거 0..2.2
            // (12 스탑)는 photometric 렌더러에서는 8bit sRGB 가 "암부 30계조 + 명부 6계조"
            // 를 동시에 담을 수 없는 정보량이다(옛 로그-직결 렌더링에서만 가능했던 분포).
            // 이 가드의 목적(chroma 재결합 + AutoLevels 알파 팽창 회귀)은 램프 깊이와 무관.
            let density = Double(x) / Double(width - 1) * 1.6
        ...
        // 2026-07-04: 자동 보정이 opt-in 으로 바뀜. 이 가드는 chroma 재결합 + AutoLevels
        // 상호작용(알파 팽창 나눗셈)의 회귀 방지가 목적이므로 원래 감시하던 스택을 명시적으로 켠다.
