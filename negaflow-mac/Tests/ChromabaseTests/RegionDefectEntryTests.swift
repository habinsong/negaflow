...
// SoftwareDefectRemoval.detectComponents / repair (영역 결함 제거 분리 진입점) 검증.
//  1) build 보존: buildLabeled→renderMask 가 기존 build 와 동일 마스크(정수 비교).
//  2) end-to-end: 검출→복원으로 결함 제거 + 배경 보존, 좌표 정합, 클릭 제외 반영.
final class RegionDefectEntryTests: XCTestCase {
    ...
    func testLabeledRenderMatchesBuild() {
        ...
        let built = DefectComponentMask.build(width: w, height: h, dust: dust, scratch: scratch,
        ...
        let rendered = DefectComponentMask.renderMask(field, excluded: [], maxHoleArea: maxDustArea, dustDilate: dilate)
    ...
    func testDetectComponentsThenRepairRemovesScratch() {
        ...
        let params = SoftwareDefectParameters(strength: 1, dustSensitivity: 0.6, scratchSensitivity: 0.7, protectDetail: 0.6)
        ...
        let maskBytes = DefectComponentMask.renderMask(field, excluded: [], maxHoleArea: w * h, dustDilate: 2)
        let out = render(SoftwareDefectRemoval.repair(image: img, roi: roi, mask: ciImage(maskBytes, w, h)), w, h)
    ...
    func testExcludedComponentNotRepaired() {
        ...
        let maskBytes = DefectComponentMask.renderMask(field, excluded: [excludeID], maxHoleArea: w * h, dustDilate: 2)
        let out = render(SoftwareDefectRemoval.repair(image: img, roi: roi, mask: ciImage(maskBytes, w, h)), w, h)
    ...
    //    Debug(-Onone)는 픽셀 루프가 수배 느리므로 평소엔 skip; DEFECT_PERF=1 일 때만 측정한다.
    func testDetectionPerformanceLargeROI() throws {
        try XCTSkipUnless(ProcessInfo.processInfo.environment["DEFECT_PERF"] != nil,
                          "성능 측정은 DEFECT_PERF=1 + Release(-c release)에서만 의미가 있다")
        ...
        let roi = CGRect(x: 0, y: 0, width: w, height: h)
        let t0 = Date()
        let field = SoftwareDefectRemoval.detectComponents(in: img, roi: roi, parameters: params, tileMax: 1400, halo: 48)
    ...
    //    과거 버그: 영역 결함 제거 가 ROI 를 "결함 보증"으로 보고 brush 의 공격 임계 + ROI×0.6 면적 상한을
    ...
    func testDetectComponentsDoesNotExplodeOnGrainGradient() {
        ...
        let params = SoftwareDefectParameters(strength: 1, dustSensitivity: 1.0, scratchSensitivity: 1.0, protectDetail: 0.6)
        let field = SoftwareDefectRemoval.detectComponents(in: img, roi: CGRect(x: 0, y: 0, width: w, height: h), parameters: params)
    ...
    func testRepairReturnsFullImageNotJustROI() {
        ...
        guard let repaired = SoftwareDefectRemoval.repairComponents(image: img, roi: roi, field: field, excluded: []) else {
    ...
    func testRepairComponentsAlignsMaskToIntersectedROI() throws {
        let w = 32, h = 24, base = 120
        let defectX = 3, defectY = 12
        var px = gray(w, h, base)
        let defectOffset = (defectY * w + defectX) * 4
        px[defectOffset] = 210
        px[defectOffset + 1] = 210
        px[defectOffset + 2] = 210
        let image = ciImage(px, w, h)
        let requestedROI = CGRect(x: -8, y: 4, width: 24, height: 16)
        let clippedROI = requestedROI.integral.intersection(image.extent)
        let fieldWidth = Int(clippedROI.width)
        let fieldHeight = Int(clippedROI.height)
        let localX = defectX - Int(clippedROI.minX)
        let localY = defectY - Int(clippedROI.minY)
        let pixel = localY * fieldWidth + localX
        var labels = [Int32](repeating: -1, count: fieldWidth * fieldHeight)
        labels[pixel] = 0
        ...
        let repaired = try XCTUnwrap(
            SoftwareDefectRemoval.repairComponents(
                image: image,
                roi: requestedROI,
                field: field,
                excluded: [],
        ...
        XCTAssertLessThan(abs(lum(out, w, defectX, defectY) - base), 24)
        XCTAssertLessThan(abs(lum(out, w, 24, defectY) - base), 5)
    }

    func testMaskDataRepairMatchesImageMaskPathAndRejectsInvalidSize() throws {
        let w = 96, h = 72
        var px = [UInt8](repeating: 255, count: w * h * 4)
        for y in 0..<h {
            for x in 0..<w {
                let offset = (y * w + x) * 4
                px[offset] = UInt8(70 + (x * 80 / w))
                px[offset + 1] = UInt8(82 + (y * 64 / h))
                px[offset + 2] = UInt8(96 + ((x + y) * 52 / (w + h)))
            }
        }
        ...
        let image = ciImage(px, w, h)
        let roi = CGRect(x: 0, y: 0, width: w, height: h)
        let maskBytes = SoftwareDefectRemoval.componentMaskBytes(
            field: field,
            excluded: [],
            dustDilate: 1
        )
        let baseline = SoftwareDefectRemoval.repair(
            image: image,
            roi: roi,
        ...
        let optimized = try XCTUnwrap(
            SoftwareDefectRemoval.repair(
                image: image,
                roi: roi,
                maskRGBA8: Data(maskBytes),
                width: w,
                height: h
            )
        )

        XCTAssertEqual(render(optimized, w, h), render(baseline, w, h))
        XCTAssertNil(SoftwareDefectRemoval.repair(
            image: image,
            roi: roi,
            maskRGBA8: Data(maskBytes.dropLast()),
            width: w,
            height: h
        ))
    }
    ...
    func testFatDustDetectedAtHighSensitivity() {
        ...
        let img = ciImage(px, w, h)
        let roi = CGRect(x: 0, y: 0, width: w, height: h)
        let high = SoftwareDefectRemoval.detectComponents(in: img, roi: roi,
            parameters: SoftwareDefectParameters(strength: 1, dustSensitivity: 1.0, scratchSensitivity: 1.0, protectDetail: 0.6))
    ...
    //     복원으로 되살아나지 않는다. 브러시↔가이드 통합(cleaned raw = 원본 + edits 순차 적용)이
    ...
    func testSequentialMaskRepairsAccumulate() {
        ...
        let after1 = SoftwareDefectRemoval.repair(image: img, roi: roi, mask: mask(39..<43))
        let after2 = SoftwareDefectRemoval.repair(image: after1, roi: roi, mask: mask(119..<123))
