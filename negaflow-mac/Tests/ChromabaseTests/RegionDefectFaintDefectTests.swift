...
final class RegionDefectFaintDefectTests: XCTestCase {
    ...
    // B) grain-safe 경계: strong 코어가 전혀 없는 균일 저대비 선은 "기하 증거(길이)가 부족하면"
    //    검출되지 않아야 한다. (원래는 길이와 무관하게 weak-only 전면 기각이었으나, 매우 길고 가는
    //    초저대비 스크래치는 LSD/a-contrario 기하 증거로 채택하도록 바뀌었다 —
    //    RegionDefectThinScratchTests.testUniformVeryFaintLongThinScratchDetected 가 그 요구를 고정한다.
    //    여기서는 남은 grain-safe 경계, 즉 증거 하한(길이 80px) 미만의 weak-only 기각을 고정한다.)
    func testUniformSubThresholdShortLineNotDetected() {
        let w = 240, h = 240, base = 120
        let x = 120, y0 = 90, y1 = 150   // 길이 61 < 기하 증거 하한(80)
        var px = bg(w, h, base)
        ...
        print(String(format: "[faint] uniform sub-threshold short line coverage=%.0f%% comps=%d", coverage * 100, field.components.count))
        XCTAssertLessThan(coverage, 0.30, "기하 증거가 부족한 weak-only 선은 검출되면 안 된다(grain-safe 경계, \(Int(coverage * 100))%)")
    }
}
