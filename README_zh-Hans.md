...
<p align="center">
  ...
  <a href="#下载"><img src="https://img.shields.io/badge/version-1.1.6-EF8B26" alt="版本 1.1.6"></a>
...
**negaflow** 可以对扫描或相机翻拍的胶片进行显影，支持彩色、黑白、负片和正片。整理照片、显影和印相都在同一个应用里完成。调整值单独保存，原始文件保持不变。

显影引擎叫 **Chroma Engine**，**GrainMend** 负责修复灰尘和划痕。没有扫描仪也可以导入图像文件，显影后导出。连接扫描仪需要单独安装插件。
...
| 安装文件 | 适用环境 |
|---|---|
| `negaflow-1.1.6-mac-universal.pkg` | macOS 14 及以上，Apple Silicon 和 Intel |
| `negaflow-1.1.6-mac-arm64.pkg` | macOS 14 及以上，仅 Apple Silicon |
| `negaflow-1.1.6-win-x64.exe` | Windows 11 24H2 或更高，x64 |
...
- 七种印相版式、纸张预览、照片用与 ISO 纸张，以及 C-print 功能

## Chroma Engine

**Chroma Engine** 负责胶片的反相和显影。

显影负片前，先在未曝光区域测量片基。自动测量偏掉的地方，用吸管点一下或者调 RGB 值就行。
...
自动和引导先检查周围纹理的方向和结构，再填补缺陷。把栏杆或砖缝误当成划痕擦掉，会损坏照片。
...
内置 15 个扫描仪配置文件，用我自己拍摄的胶片制作，共记录了 928 个数据点。
